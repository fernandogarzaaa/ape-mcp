...
1. **`ape_beliefs` + `ape_genome` promised but not registered.** The tool
   map and `skills/APE/SKILL.md` referenced them; the server returned
   `unknown_tool`. â†’ Registered; now call the real vendored `adam-mcp` binary.
2. **Memory tools were stubs.** `ape_remember/recall` returned a note instead
   of persisting. â†’ New `src/adam-client.js` spawns the vendored
   `adam-mcp` binary over line-delimited JSON-RPC (`initialize` â†’ `tools/call`)
   ...
   embedded record. State lives under `$APE_DATA_DIR` (`adam_memory.db`,
   `adam_genome.json`).
3. **`ape_mcp_eval` missing.** EVE's `mcp-eval` (schema/conformance/fuzz
   oracles) exists in the vendored CLI but wasn't surfaced. â†’ Registered and wired
   to `node vendors/eve/bin/eve.js mcp-eval <target>`.
4. **Skein orchestration needed `PYTHONPATH` manually.** `ape_orchestrate`
   shells out to `python -m skein.cli`; from a bare `node ape-mcp.js` the
   vendored package wasn't importable. â†’ Dispatch now injects
   `PYTHONPATH=vendors/skein/src` automatically (same will apply to eve-miro).
5. **No ledger surface.** `ape_audit_claim` wrote hash-chained entries but the
   console Ledger tab was a static placeholder. â†’ `audit_claim` appends a summary
   to `.ape/ledger.jsonl`; console `/api/ledger` + Ledger tab now render it.
6. **Test/audit blind spot â€” PowerShell `$args` automatic variable.** `live-audit.ps1`
...
- **`ape_world_simulate` + eve-miro Python side.** Returns the provenance
  contract but doesn't run MIRO; needs Python 3.12 venv + deps. Add
  `APE setup` to provision `.ape/venv` (eve-miro fabric) â€” fail-closed
  today is honest, but a setup step makes it usable.
- **`ape_compare` is a placeholder** (returns a note). Wire to
  `vendors/genesis compare/regression` like `audit_claim`.
- **`ape_evolve` governance note only.** MRTR confirm works; `accept` should
...
- **Local dev rebuild gap:** re-vendoring sources doesn't rebuild `dist/` until the
  updater runs; add a `npm run vendor` â†’ auto-rebuild note (CI updater already does it).

## GitHub Actions failures (2026-09-17) â€” diagnosed & fixed
...
1. **`adam-binaries.yml` "invalid workflow file" (0s, every push).** `with: { targets: ${{ matrix.target }} }`
   used flow-style mapping containing a brace expression â€” the `}}` collides with the flow map's
...
2. **`auto-update` fails at "Open update PR": `GitHub Actions is not permitted to create or approve
   ...
   `secrets.APE_SYNC_TOKEN || github.token` (PAT in APE repo) and docs tell you the
   one-click alternative (Settings â†’ Actions â†’ General â†’ allow PR creation). The sync itself â€”
   resolve â†’ clone â†’ re-vendor â†’ rebuild â†’ license gate â†’ verify â€” all **succeeded** in that run.
3. **Dependabot "github-actions in / - Update" failed**: cascade of (1) â€”
...
`npm test` 12/12 Â· `scripts/live-audit.ps1` **17/17** (real HTTP calls, real ADAM
memory storeâ†’recall, MRTR confirm, task lifecycle) Â· `smoke` 15 tools Â· `doctor` 8/8 Â·
license gate ok Â· `verify-adam` handshake ok.