...
export function digestBundle(dir) {
    ...
    for (const rel of listFiles(dir)) {
        ...
        if (rel === "attestations" || rel.startsWith("attestations/"))
            continue;
...
export function attestBundle(dir, signer, privateKeyPem) {
    ...
    try {
        ...
        attestation.key_fingerprint = fingerprint(pubPem);
    }
    catch {
 ...
 */
export function verifyBundle(dir, publicKeys = []) {
    const digest = digestBundle(dir);
    ...
    const checks = files.map((f) => {
        const att = JSON.parse(readFileSync(join(attDir, f), "utf8"));
