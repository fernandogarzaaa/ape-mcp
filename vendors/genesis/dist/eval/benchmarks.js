...
export function resolveRegistry(customDir) {
    const candidates = [];
    if (customDir)
        candidates.push(resolve(customDir));
    ...
    throw new BenchmarkError(`no benchmark registry found (tried ${candidates.join(", ")}); pass --registry <dir>`);
}
export function listBenchmarks(registryDir) {
    ...
    for (const entry of readdirSync(dir).sort()) {
        ...
        try {
            if (!statSync(sub).isDirectory() || !statSync(specFile).isFile())
                continue;
        }
        catch {
            continue;
...
export function loadBenchmark(name, registryDir) {
    const dir = resolveRegistry(registryDir);
...
function countDatasetLines(spec, sub) {
    const p = spec.dataset.path;
    if (!p)
        return spec.dataset.inline?.length ?? -1;
