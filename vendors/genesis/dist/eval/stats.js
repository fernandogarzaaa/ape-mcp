...
export { wilson };
export function describe(values, metric) {
...
export function describeRate(successes, total, metric) {
    if (total === 0)
        return null;
...
export function bootstrapMeanCI(values, resamples = 2000, level = 0.95, seed = 1) {
    const v = [...values];
    if (v.length === 0)
 ...
 */
export function pairedCompare(baseline, treatment, metric) {
    ...
    if (keys.length === 0)
        return null;
    const diffs = keys.map((k) => t.get(k) - b.get(k));
    const bMean = keys.reduce((a, k) => a + b.get(k), 0) / keys.length;
    const tMean = keys.reduce((a, k) => a + t.get(k), 0) / keys.length;
...
export function checkThreshold(value, expr) {
