...
export function loadBundleMetrics(dir) {
    ...
    for (const cand of [join(dir, "treatment", "metrics.json"), join(dir, "baseline", "metrics.json")]) {
        try {
            metrics = JSON.parse(readFileSync(cand, "utf8"));
            break;
        }
        catch {
            // try next
...
export function compareBundles(aDir, bDir) {
...
export function renderComparison(aDir, bDir) {
...
export function checkRegression(baseDir, candDir, config) {
