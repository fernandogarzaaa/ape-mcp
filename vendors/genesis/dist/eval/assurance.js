...
import { runAudit } from "../assurance/audit.js";
...
export async function assureEvaluator(spec, options = {}) {
    ...
    for (const probe of probes) {
        const task = dataset.tasks.find((t) => t.id === probe.task_id);
        let observed;
        let detail;
        try {
...
function empty(reason, dataset, evaluator) {
...
function exploits(task, gold, allGolds, isRetrieval, wrongGoldOk = true) {
    const out = [];
    ...
    out.push({
        ...
        output: isRetrieval && gold && typeof gold === "object"
            ? { retrieved_ids: [...gold.retrieved_ids, ...JUNK] }
            : `${normStr(gold)}\n${JUNK.join(" ")}`,
...
function idArray(v) {
    ...
    if (v && typeof v === "object") {
        const r = v.retrieved_ids;
        if (Array.isArray(r))
            return r.map(String);
    }
    return null;
}
function stress(task, gold) {
...
function judgeOutcome(kind, observed) {
    if (kind === "stress")
        return observed === "accept" ? "correct" : "stress_note";
    if (kind === "control")
        return observed === "accept" ? "correct" : "false_reject";
...
function decideAssurance(falseAccepts, falseRejects, controls) {
    if (controls === 0) {
...
function buildAssuranceFindings(results) {
    const findings = [];
 ...
 */
export async function auditSpecEvaluatorAgainstSuite(spec, suiteName) {
    ...
    const judge = {
        ...
        judge: async (probe) => {
            ...
            try {
                ...
                const observed = obs.passed === true ? "accept" : obs.passed === false ? "reject" : "error";
                return {
                ...
                return {
                    ...
                    duration_ms: Date.now() - started, note: error.message,
                };
            }
        },
    };
...
export function decideTrust(system, evaluator) {
    const reasons = [];
    if (system === "INVALID") {
...
export function renderAssurance(a) {
    const L = [];
    const pct = (v) => v.n === 0 ? "n/a (no cases)" : `${(v.point * 100).toFixed(1)}% [${(v.low * 100).toFixed(1)}–${(v.high * 100).toFixed(1)}%, n=${v.n}]`;
    L.push("");
    ...
    if (a.abstains > 0)
        L.push(`  Abstained      ${a.abstains} probe(s) — safe, but verdicts cannot rely on them`);
    L.push("");
    if (a.findings.length > 0) {
        ...
        for (const f of a.findings) {
            ...
            if (f.probes.length > 0)
                L.push(`    probes: ${f.probes.slice(0, 8).join(", ")}${f.probes.length > 8 ? "…" : ""}`);
        }
        L.push("");
    }
    L.push("WHY");
...
export function renderTrust(systemLabel, systemSummary, assurance, trust) {
    const L = [];
    L.push("");
