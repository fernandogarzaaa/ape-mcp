 ...
 */
export function decideGate(result, assurance, trust, forbidden = {}) {
    const breaches = [];
    const reasons = [];
    const byName = new Map(result.arms[0]?.metrics.map((m) => [m.metric, m]) ?? []);
    ...
    if (system === "INVALID")
        breaches.push({ kind: "system", detail: "evaluation specification is invalid" });
    if (system === "FALSIFIED")
        breaches.push({ kind: "system", detail: `claim failed: ${result.verdict.summary}` });
    ...
    const insufficient = system === "INCONCLUSIVE" ||
        system === "UNTESTED" ||
    ...
    if (breaches.length > 0) {
        ...
        decision = "RELEASE";
        reasons.push(`Claim ${system === "SUPPORTED" ? "held" : system} under a ${assurance.verdict} evaluator with trust ${trust}; ` +
...
export function renderGate(label, forbidden, gate) {
    const L = [];
    L.push("");
    ...
    if (names.length === 0)
        L.push("  (no forbidden ceilings declared — add spec.gate.forbidden)");
