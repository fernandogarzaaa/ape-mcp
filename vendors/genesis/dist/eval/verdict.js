...
export function decideVerdict(input) {
    ...
    if (typeof minSamples === "number" && input.sampleSize < minSamples) {
        return verdict(fallback, input, `Only ${input.sampleSize} samples collected; methodology requires >= ${minSamples}.`, hypothesisResults, null);
    }
    ...
    if (allSatisfied) {
        ...
        return verdict("SUPPORTED", input, `All ${hypothesisResults.length} hypothesi(es) held (${primary?.metric} ${primary?.operator} ${primary?.threshold}).`, hypothesisResults, null);
    }
    return verdict("FALSIFIED", input, `At least one hypothesis failed: ${hypothesisResults.filter((r) => r.satisfied === false).map((r) => `${r.metric} ${r.operator} ${r.threshold} (observed ${formatNum(r.observed)})`).join("; ")}.`, hypothesisResults, null);
}
function verdict(v, input, summary, hypothesisResults, _extra) {
    void _extra;
