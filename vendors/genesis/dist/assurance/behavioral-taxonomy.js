 ...
 */
export const BEHAVIORAL_DOMAINS = ["behavioral"];
export const BEHAVIORAL_DEFECT_CLASSES = [
    "zero_interaction_success",
    "incidental_label_match",
];
export const BEHAVIORAL_TAXONOMY = {
    zero_interaction_success: {
        id: "zero_interaction_success",
        domain: "behavioral",
        title: "Zero-interaction success",
        defect: "Marks the goal achieved from the very first perception, before the operator has taken a single action.",
        exploit: "Choose a success signal that matches incidental text already visible on the start screen — an app name, a tagline, a heading — so the oracle reports success with no task performed at all.",
    },
    incidental_label_match: {
        id: "incidental_label_match",
        domain: "behavioral",
        title: "Incidental label match",
        defect: "Marks the goal achieved because a success signal matches the visible LABEL of an interactive element the operator has not yet activated, rather than the outcome of activating it.",
        exploit: "Choose a success signal that is also the label of a button or link on the path to the goal — often a literal substring of the goal description, and so a natural rather than adversarial choice — so the oracle reports success on arrival at that screen, without the labeled action ever being taken.",
    },
};
export function behavioralDescriptorsFor(domain) {
    return BEHAVIORAL_DEFECT_CLASSES.map((id) => BEHAVIORAL_TAXONOMY[id]).filter((d) => d.domain === domain);
}
//# sourceMappingURL=behavioral-taxonomy.js.map