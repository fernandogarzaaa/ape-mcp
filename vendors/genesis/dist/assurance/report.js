...
export function renderAudit(record, options = {}) {
    ...
    const clean = metrics.by_class.filter((c) => c.false_accepts === 0 && c.unresponsive === 0 && c.errors === 0);
    if (clean.length > 0) {
    ...
    if (options.verbose) {
        ...
        for (const result of results) {
            lines.push(`  ${MARK[result.outcome]} ${result.probe_id.padEnd(28)} ` +
    ...
    lines.push("WHY");
    for (const reason of conclusion.rationale)
        lines.push(`  ${wrap(reason, 76, "  ")}`);
    lines.push("");
    lines.push("SCOPE: this audit bounds only the defect classes the suite probes. A SOUND verdict " +
...
function renderFinding(lines, finding) {
