...
import { loadSpecFile, validateSpec } from "../eval/spec.js";
...
function fail(message) {
    return { content: [{ type: "text", text: `genesis: ${message}` }], isError: true };
}
function freshOutDir(prefix) {
...
export function createMcpServer() {
    ...
    server.registerTool("evaluate", {
        ...
        description: "Run a declarative Genesis evaluation (claim → dataset → subject → evaluator → metrics → verdict). " +
        ...
        try {
            const spec = validateSpec(args.spec, "<mcp>");
    ...
    server.registerTool("audit_evaluator", {
        title: "Audit an evaluator",
        description: "Assure the evaluator from a spec with dataset-derived exploit/control probes " +
    ...
    server.registerTool("trust", {
        title: "Evaluate and assure (combined trust)",
        description: "Run the evaluation AND assure its evaluator, combining both into TRUSTED | UNTRUSTED | INCONCLUSIVE. " +
    ...
    server.registerTool("list_benchmarks", {
        ...
        try {
            ...
            return ok(infos.map((b) => `${b.name} v${b.version} — ${b.task_count} tasks — ${b.description}`).join("\n"), { benchmarks: infos });
        }
        catch (error) {
            return fail(error.message);
        }
    });
    server.registerTool("run_benchmark", {
        title: "Run a benchmark",
        description: "Run your system (command template with {task_file}, or a built-in inline subject) " +
        ...
        try {
            ...
            const spec = {
                ...
                subject: args.subjectCommand
                    ...
                    : { name: "candidate", inline: args.subjectInline },
    ...
    server.registerTool("check_regression", {
        title: "Regression gate",
        description: "Gate a candidate bundle against a baseline (quality max drop, latency/cost max increase). " +
    ...
    server.registerTool("gate", {
        ...
        description: "Run the evaluation, assure its evaluator, and bind both to forbidden capability ceilings " +
    ...
    server.registerTool("show_claim", {
        title: "Show claim",
        ...
        try {
            ...
            if (!spec.claim)
                return ok("No claim in this spec.", { claim: null });
            return ok(`claim ${spec.claim.id}: ${spec.claim.statement}`, { claim: spec.claim });
        }
        catch (error) {
            return fail(error.message);
        }
