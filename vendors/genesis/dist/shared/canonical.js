...
function write(value, seen, path) {
    ...
    switch (typeof value) {
        ...
        case "number":
            if (!Number.isFinite(value)) {
                throw new CanonicalizationError(`${path}: non-finite number (${String(value)}) cannot be canonicalized`);
            }
        ...
        case "function":
        case "symbol":
            throw new CanonicalizationError(`${path}: ${typeof value} cannot be canonicalized (it would be silently dropped)`);
    }
    const obj = value;
    if (seen.has(obj)) {
    ...
    try {
        ...
        const members = keys.map((key) => `${JSON.stringify(key)}:${write(record[key], seen, `${path}.${key}`)}`);
        return `{${members.join(",")}}`;
    }
    finally {
        seen.delete(obj);
 ...
 */
export function hashCanonicalExcluding(value, exclude) {
    const filtered = {};
