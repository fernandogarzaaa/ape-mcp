import { asKernelSurface } from "../browser/adapter.js";
import { hesitationMs, planClick, planSoftKeyType, planSwipe, planTap, planTyping, } from "../browser/humanizer.js";
...
import { comparePrediction, errorSnippets, inferAppTheory, passiveText, visibleText, } from "../cognition/mentalModel.js";
import { readingLoad, riskOf } from "../cognition/salience.js";
import { SimulatedClock, WALL_CLOCK } from "../core/clock.js";
...
import { resolveTaskId } from "../planning/task.js";
...
import { stripPercept } from "../trace/trace.js";
...
import { discoverJourney } from "../workflow/journeys.js";
import { CognitiveSuite, } from "./cognitiveSuite.js";
/**
...
export class EveSession {
    ...
    findingCounter = 0;
    screenshotGallery = [];
    lastShotBySignature = new Map();
    geometryCheckedSignatures = new Set();
    ...
    constructor(options) {
        ...
        let persona = typeof options.persona === "string"
        ...
        this.events = new EventBus((err, event) => this.log(`listener error on ${event}: ${String(err)}`));
        this.plugins = new PluginManager((err, plugin) => this.log(`plugin "${plugin}" error: ${String(err)}`));
    ...
    async run() {
        ...
        if (this.options.longTermMemory) {
            ...
            memory.seedFamiliarScreens(Object.values(appMemory.screens).map((s) => ({
        ...
        const llmFallbackWarnings = [];
        const recordLlmFallback = (source, reason) => {
            void this.events.emit("llm:fallback", { source, reason });
            const message = source === "cognition"
                ? `LLM cognition fallback: ${reason}`
                : `LLM critic plugin fallback: ${reason}`;
            if (llmFallbackWarnings.includes(message))
                return;
        ...
        let terminalPercept = null;
        let terminalStep = 0;
        let terminalFormFill = "empty";
        ...
        const recordGoalSignalWarning = (message) => {
            if (goalSignalWarnings.includes(message))
                return;
        ...
        try {
            ...
            while (step < this.options.maxSteps) {
                ...
                if (dropped)
                    this.log(`(mind wandered — forgot: ${dropped})`);
                ...
                const text = (adapter.capabilities.modality === "conversational"
                    ? surfaceAuthoredText(await this.kernelOf(adapter, percept))
                    : visibleText(percept)).toLowerCase();
                ...
                if (activeSignals === null) {
                    ...
                    if (retired.length > 0) {
                        recordGoalSignalWarning(`success signal(s) [${retired.join(", ")}] were already satisfied by the starting ` +
                ...
                if (activeSignals.length > 0 && activeSignals.every((s) => matchesSignal(text, s))) {
                    ...
                    if (labelOnly.length > 0) {
                        recordGoalSignalWarning(`success signal(s) [${labelOnly.join(", ")}] were satisfied only by the label of an ` +
                ...
                if (errorNow && !goals.subgoal) {
                    goals.push(createGoal("recover from the error on screen", {
                ...
                if (cognitionFallback)
                    recordLlmFallback("cognition", cognitionFallback);
                goals.tickEffort();
                ...
                this.log(`#${step} [${goals.current.description.slice(0, 40)}] ${describeAction(decision.action)} — ${decision.rationale}`);
                if (decision.action.kind === "abandon") {
                    ...
                    iterations.push(this.makeIteration(step, percept, goals, decision, null, emotion, screenshotIndex, null, formFill, errorNow, percept));
                    terminalPercept = percept;
                    terminalStep = step;
                    terminalFormFill = formFill;
                    terminalError = errorNow;
                ...
                const madeProgress = outcome.screenChanged &&
                ...
                if (suite)
                    suite.afterOutcome(decision, outcome, percept, after.percept, emotion, step);
                ...
                const iteration = this.makeIteration(step, percept, goals, decision, outcome, emotion, screenshotIndex, clickPoint, formFill, errorNow, after.percept);
            ...
            if (step >= this.options.maxSteps)
                endReason = "step-budget-exhausted";
            if (goalAchieved)
                endReason = "goal-achieved";
            if (abandoned)
                endReason = "abandoned";
            ...
            try {
                ...
                this.log(`warning: plugins.sessionEnd() failed during cleanup: ${sessionEndError instanceof Error ? sessionEndError.message : String(sessionEndError)}`);
            }
            try {
                await adapter.close();
            }
            catch (closeError) {
                this.log(`warning: adapter.close() failed during cleanup: ${closeError instanceof Error ? closeError.message : String(closeError)}`);
            }
        }
        const usage = {
        ...
        const findings = [...this.findings.values()].sort((a, b) => severityRank(a.severity) - severityRank(b.severity));
    ...
    captureScreen(percept, signature) {
        if (this.capturedScreens.has(signature))
            return;
    ...
    updateLongTermMemory(appMemory, memory, graph, ctx) {
        ...
        for (const f of frustratingFindings) {
            ...
            if (spot)
                ...
                appMemory.frustrationSpots.push({ signature: key, title: f.url, occurrences: 1 });
        }
        ...
        switch (action.kind) {
            ...
            case "doubleClick": {
                ...
                if (action.kind === "doubleClick")
                    await adapter.doubleClickAt(gesture.point);
                else
            ...
            case "type": {
                ...
                for (const key of plan.keystrokes) {
                    if (key === "\b")
                        await adapter.pressKey("Backspace");
                    else
                        await adapter.typeText(key, 0);
                    await this.pace(plan.perCharIntervalMs);
                }
                if (plan.typoCount > 0)
                    this.log(`(made and corrected ${plan.typoCount} typo(s))`);
                return gesture.point;
            }
            case "press":
                await this.pace(200);
    ...
    async pace(humanMs) {
        ...
        if (realMs >= 5)
            await new Promise((r) => setTimeout(r, Math.min(realMs, 4000)));
    }
    /* ---------------------------------------------------------------- */
    /* Findings & learning                                               */
    /* ---------------------------------------------------------------- */
     ...
     * visual audit.
     */
    runVisionChecks(percept, signature) {
        if (!this.options.adapter.capabilities.spatial)
            return;
        if (!this.geometryCheckedSignatures.has(signature)) {
            ...
            for (const issue of checkGeometry(percept, this.persona.accessibility)) {
                this.addFinding({
                    ...
                    category: issue.kind === "low-contrast" ||
    ...
    runRenderingChecks(percept) {
        ...
        for (const issue of inspectRendering(percept).issues) {
            ...
            const severity = issue.kind === "unaccounted-content" ? "major" : "minor";
            this.addFinding({
                severity,
    ...
    storeScreenshot(percept, signature) {
        if (!percept.screenshot)
            return null;
    ...
    learnFromOutcome(memory, decision, outcome, before, after) {
        const action = decision.action;
        ...
        if (action.kind === "click" && outcome.errorPerceived) {
            memory.learn({ kind: "warning", statement: `clicking "${action.target.text.trim()}" caused an error` }, 0.7);
        }
        if (action.kind === "press" && outcome.screenChanged) {
            memory.learn({ kind: "shortcut", statement: `pressing ${action.key} works here` }, 0.5);
        }
    }
    reportOutcomeFindings(decision, outcome, percept, screenshotIndex) {
    ...
    addFinding(finding) {
        ...
        const full = { ...finding, id: `F-${String(this.findingCounter).padStart(3, "0")}` };
        this.findings.set(key, full);
        void this.events.emit("finding", { finding: full });
    }
    makeIteration(step, percept, goals, decision, outcome, emotion, screenshotIndex, clickPoint, formFill, errorSignal, stateAfter) {
        return {
            step,
            timestamp: this.simClock,
            url: percept.url,
     ...
     */
    terminalStateOf(percept, step, formFill, errorSignal) {
        const text = visibleText(percept);
        return {
            url: percept.url,
            title: percept.title,
...
function retainedKnowledgeQuick(memory) {
    let sum = 0;
...
function iterationsSurpriseRate(memory) {
