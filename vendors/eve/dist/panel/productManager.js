export function buildProductPlan(input) {
    ...
    for (const issue of executive.consensusIssues) {
        ...
        const lift = forecast?.recommendedChanges.find((c) => c.change.toLowerCase().includes(issue.category))
...
function impactWeight(severity) {
    return { critical: 1, major: 0.6, minor: 0.25, info: 0.1 }[severity];
}
function effortWeight(category) {
    // Rough effort proxy: content/microcopy is cheap; navigation/IA is costly.
    if (category === "content" || category === "visual")
        return 0.5;
    if (category === "navigation" || category === "workflow")
        return 1.5;
    if (category === "accessibility")
        return 0.8;
    return 1;
}
function businessImpact(severity, agreement, lift) {
    const audience = agreement >= 0.75 ? "most users" : agreement >= 0.5 ? "many users" : "a segment of users";
    const consequence = severity === "critical"
...
function acceptanceFor(category, url) {
