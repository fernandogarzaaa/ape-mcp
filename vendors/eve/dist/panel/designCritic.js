 ...
 */
export function critiqueDesign(screens, behavioralFindings = []) {
    const items = [];
    const seen = new Set();
    const push = (item) => {
    ...
    for (const screen of screens) {
        ...
        if ((kind === "edit" || kind === "create" || kind === "settings") &&
    ...
    for (const item of items)
        byHeuristic[item.heuristic] = (byHeuristic[item.heuristic] ?? 0) + 1;
    const penalty = items.reduce((sum, i) => sum + (i.severity === "critical" ? 12 : i.severity === "major" ? 6 : 2), 0);
