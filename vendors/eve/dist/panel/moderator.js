 ...
 */
export function moderatePanel(input) {
    ...
    for (const session of sessions) {
        ...
        for (const f of session.findings) {
            ...
            if (severityRank(f.severity) < severityRank(cluster.finding.severity))
                cluster.finding = f;
            clusters.set(key, cluster);
        }
    }
    const personaCount = new Set(sessions.map((s) => s.personaName)).size;
    const consensusIssues = [...clusters.values()]
        .map((c) => ({
        ...
        .sort((a, b) => severityRank(a.severity) - severityRank(b.severity) || b.agreement - a.agreement);
...
function findDisagreements(sessions, issues, personaCount) {
    const out = [];
...
function buildPriorities(consensus, forecast, critique) {
    const priorities = [];
    for (const issue of consensus
        .filter((i) => i.severity === "critical" || i.severity === "major")
        .slice(0, 4)) {
        priorities.push(`Fix "${issue.title}" (${issue.severity}, ${Math.round(issue.agreement * 100)}% of personas) at ${issue.url}`);
    }
    if (forecast) {
...
function buildExecutiveSummary(args) {
    ...
    const grade = meanOverallScore >= 80
        ? "strong"
        : meanOverallScore >= 65
            ? "acceptable"
            : meanOverallScore >= 50
