...
export function topKAgreement(humanChoices, eveTopK, equals = Object.is) {
    if (humanChoices.length === 0 || humanChoices.length !== eveTopK.length)
        return 0;
    let hits = 0;
    for (let i = 0; i < humanChoices.length; i++) {
        if (eveTopK[i].some((c) => equals(c, humanChoices[i])))
            hits += 1;
    }
 ...
 */
export function spearmanRankCorrelation(xs, ys) {
 ...
 */
export function transitionDivergenceL1(a, b) {
    ...
    let dist = 0;
    for (const k of keys)
        dist += Math.abs((a.get(k) ?? 0) / sumA - (b.get(k) ?? 0) / sumB);
    return dist;
}
/**
 ...
 */
export function meanLogDurationError(predictedMs, actualMs) {
    if (predictedMs.length !== actualMs.length)
        return null;
    let sum = 0;
    let n = 0;
