...
function redactEmails(text) {
    return text
        ...
        let end = rest.length;
        while (end > 0 && TRAIL_PUNCT_CHARS.has(rest[end - 1]))
            end -= 1;
 ...
 */
export function sanitizeTraceUrl(url) {
    try {
        const u = new URL(url);
...
export function sanitizeHumanTrace(trace) {
    return {
        ...trace,
...
export function sanitizeHumanStudy(study) {
    return {
        ...study,
...
export function sanitizeCanonicalState(state) {
    return {
        ...state,
...
export function sanitizeHumanStep(step) {
    return {
        ...step,
        ...
        ...(step.selfReport
            ? {
                ...
                selfReport: Object.fromEntries(Object.entries(step.selfReport).filter(([k]) => !isSecretFieldName(k))),
            }
            : {}),
    };
}
