...
function summarizeHuman(study) {
    ...
    for (const t of traces) {
        ...
        if (screen)
            perScreenAbandon.set(screen, (perScreenAbandon.get(screen) ?? 0) + 1 / n);
    }
 ...
 */
export function calibrate(human, eve) {
    const h = summarizeHuman(human);
    const e = summarizeEve(eve);
    const notes = [];
    const behaviorSimilarity = clamp01(1 -
        0.5 *
    ...
    const frustrationAlignment = h.meanFrustration !== null && e.meanFrustration !== null
    ...
    const confidenceAlignment = h.meanConfidence !== null && e.meanConfidence !== null
    ...
    if (frustrationAlignment !== null)
        components.push({ value: frustrationAlignment, weight: 0.1 });
    if (confidenceAlignment !== null)
        components.push({ value: confidenceAlignment, weight: 0.1 });
...
export function importHumanStudy(raw) {
    if (typeof raw !== "object" || raw === null)
        throw new Error("Human study must be an object.");
    const obj = raw;
    if (!Array.isArray(obj.traces))
        throw new Error("Human study must have a `traces` array.");
    const traces = obj.traces.map((t, i) => {
        if (typeof t !== "object" || t === null)
            throw new Error(`Trace ${i} must be an object.`);
        const tr = t;
