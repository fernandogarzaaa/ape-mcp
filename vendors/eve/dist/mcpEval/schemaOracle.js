...
function propertyIsDescribable(prop) {
    if (!isRecord(prop))
        return false;
...
export function checkToolSchemas(tools) {
    const findings = [];
    ...
    for (const tool of tools) {
        ...
        if (!isRecord(schema)) {
            findings.push(makeFinding({
                ...
                description: "MCP tools must advertise an inputSchema; without one clients cannot build valid calls.",
            ...
            if (schema.type !== "object") {
                findings.push(makeFinding({
                    ...
                    description: 'The MCP specification requires every tool inputSchema to have "type": "object".',
                ...
                if (!Array.isArray(required) || required.some((r) => typeof r !== "string")) {
                    ...
                    if (dangling.length > 0) {
                        findings.push(makeFinding({
                            ...
                            description: "Every required entry must name a declared property; dangling entries make the schema unsatisfiable by construction.",
            ...
            const undescribable = Object.entries(properties).filter(([, prop]) => !propertyIsDescribable(prop));
            if (undescribable.length > 0) {
                findings.push(makeFinding({
                    severity: "minor",
                    category: "mcp.schema-quality",
                    title: `"${name}" has unconstrained properties`,
                    description: "Properties with no type/enum/$ref/combinator accept anything; clients cannot render or validate them.",
                    evidence: undescribable.map(([p]) => `property "${p}" has no type or constraint`),
                    tool: name,
                }));
            }
            const undocumentedProps = Object.entries(properties).filter(([, prop]) => !isRecord(prop) || typeof prop.description !== "string");
            if (undocumentedProps.length > 0) {
                findings.push(makeFinding({
                    severity: "info",
                    category: "mcp.schema-quality",
                    title: `"${name}" has undocumented properties`,
                    description: "Property descriptions are how operators (human or agent) learn what an argument means.",
        ...
        if (description.length === 0) {
            findings.push(makeFinding({
                ...
                description: "The description is the only discovery channel a client has for what a tool does and when to use it.",
        ...
        if (hints === null) {
            findings.push(makeFinding({
                ...
                description: "readOnlyHint/destructiveHint/idempotentHint/openWorldHint let clients gate calls safely; their absence forces callers to assume the worst.",
            ...
            if (DESTRUCTIVE_NAME.test(name)) {
                if (hints.destructiveHint === false) {
                    findings.push(makeFinding({
                        ...
                        description: "The tool name suggests irreversible change while destructiveHint is false; one of them is lying to the client.",
                    ...
                    findings.push(makeFinding({
                        ...
                        description: "Clients that gate destructive calls on hints cannot protect the user here.",
            ...
            if (READONLY_NAME.test(name)) {
                if (hints.readOnlyHint === false) {
                    findings.push(makeFinding({
                        ...
                        description: "The tool name suggests a pure read while readOnlyHint is false; one of them misleads the client.",
...
//# sourceMappingURL=schemaOracle.js.map