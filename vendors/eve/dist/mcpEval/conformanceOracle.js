...
export async function checkConformance(conn) {
    const findings = [];
    const capabilities = conn.serverCapabilities;
    const toolsCapability = capabilities && typeof capabilities === "object"
        ? capabilities.tools
        : undefined;
    ...
    if (!info?.name) {
        findings.push(makeFinding({
            ...
            description: "The initialize result should carry serverInfo with a name and version; clients show this to users deciding whether to trust the server.",
    ...
    if (toolsCapability === undefined) {
        findings.push(makeFinding({
            ...
            description: "A server that answers tools/list must declare capabilities.tools at initialize; spec-conformant clients may not even attempt tools/list otherwise.",
        ...
        try {
            ...
            if (outcome.isError) {
                findings.push(makeFinding({
                    ...
                    description: "Calling a nonexistent tool should fail at the protocol layer (-32602 invalid params); an isError result makes a routing bug indistinguishable from a tool failure.",
                    evidence: [
                ...
                findings.push(makeFinding({
                    ...
                    description: "The server answered a call for a tool it does not advertise as if it were a success — a serious routing/validation defect.",
                    evidence: [
...
//# sourceMappingURL=conformanceOracle.js.map