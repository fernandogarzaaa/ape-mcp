...
import { connectMcpServer } from "../surface/mcpClient.js";
import { checkConformance } from "./conformanceOracle.js";
import { fuzzTools } from "./fuzzOracle.js";
import { checkToolSchemas, resetFindingIds } from "./schemaOracle.js";
import { DIMENSION_FOR_CATEGORY, MCP_DIMENSIONS, } from "./types.js";
 ...
 */
function scoreDimension(dimension, findings) {
 ...
 * stripped if present.
 */
export async function evaluateMcpServer(target, options = {}) {
    ...
    try {
        ...
        const findings = [...checkToolSchemas(tools)];
...
export function renderMcpEvalMarkdown(report) {
    const lines = [];
    lines.push(`# MCP evaluation — ${report.target}`);
    lines.push("");
    lines.push(report.server
    ...
    if (report.fuzz) {
        const f = report.fuzz;
        lines.push("");
        lines.push(`Fuzzing: ${f.calls} call(s) over ${f.toolsFuzzed} tool(s) — ${f.protocolErrors} protocol errors, ${f.errorResults} error results, ${f.acceptedInvalid} accepted-invalid, ${f.hangs} hang(s), ${f.crashes} crash(es)`);
    }
    lines.push("");
    lines.push("## Findings");
