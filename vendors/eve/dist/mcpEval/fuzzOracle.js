...
import { isConnectionClosedError, isTimeoutError, } from "../surface/mcpClient.js";
...
function propertyType(schema, field) {
    const properties = schema.properties;
    if (!isRecord(properties))
        return null;
    const prop = properties[field];
    if (!isRecord(prop) || typeof prop.type !== "string")
        return null;
    return prop.type;
}
...
function buildCases(tool, options) {
    ...
    for (const field of declared)
        skeleton[field] = tameValue(propertyType(schema, field));
    for (const field of declared) {
 ...
 * headline finding.
 */
export async function fuzzTools(conn, tools, options = {}) {
    ...
    const rng = createRng(typeof options.seed === "number" ? options.seed : seedFromString(String(options.seed ?? 1)));
    const findings = [];
    const stats = {
        toolsFuzzed: 0,
        calls: 0,
    ...
    for (const tool of tools) {
        if (conn.closed || stats.crashes > 0)
            break;
            ...
            switch (outcome.kind) {
                ...
                case "hang":
                    ...
                    findings.push(makeFinding({
                        ...
                        description: "An unbounded call is a denial-of-service vector for any client; servers should reject bad input fast.",
                ...
                case "crash":
                    ...
                    findings.push(makeFinding({
                        ...
                        description: "The transport died under an adversarial call. A single malformed request must never take the whole server down.",
                        evidence: [
                            `case: ${fuzzCase.label}`,
                        ...
                        tool: name,
                        recommendation: "Validate arguments before use and isolate per-request failures from server lifecycle.",
                    }));
                    break;
            }
        }
        if (acceptedLabels.length > 0) {
            findings.push(makeFinding({
                ...
                description: "Inputs that violate the advertised schema were accepted without a protocol error or an error result — clients cannot trust the schema as a contract.",
                evidence: acceptedLabels.map((label) => `accepted: ${label}`),
                tool: name,
                recommendation: "Validate arguments against the input schema and reject violations with a JSON-RPC -32602 error.",
            }));
        }
    }
    return { findings, stats };
}
async function classifyCall(conn, name, args, timeoutMs) {
    try {
...
//# sourceMappingURL=fuzzOracle.js.map