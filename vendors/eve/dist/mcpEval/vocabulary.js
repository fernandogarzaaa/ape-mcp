...
const TEXTUAL_ONLY = ["textual"];
const DIMENSION_DESCRIPTIONS = {
    "mcp.schemaQuality": "Correctness and descriptive quality of advertised tool schemas (JSON Schema validity, required-field consistency, description quality, annotation honesty).",
    "mcp.robustness": "Behavior under malformed, boundary and oversized tool inputs (protocol-error rejection, no crashes, no hangs).",
    "mcp.conformance": "Protocol conformance: initialize handshake, capability declaration, ping, and correct error codes for unknown tools.",
};
...
export function registerMcpVocabulary() {
    if (registered)
        return;
    registered = true;
    ...
    if (!actionVerbRegistry.has("mcp.invoke")) {
        registerActionVerb({
            id: "mcp.invoke",
            description: "Invoke an MCP tool with arguments (projected as a form submission on the MCP surface).",
            appliesTo: TEXTUAL_ONLY,
        });
    }
}
