 ...
 */
import { webPerceptFromVisualKernel, } from "../core/kernel.js";
import { layoutTextFrame } from "./textFrame.js";
/** Project any kernel percept into the deprecated web view. */
export function webPerceptFromKernel(kernel) {
    if (kernel.modality === "visual")
        return webPerceptFromVisualKernel(kernel);
    if (kernel.modality === "document")
        return webPerceptFromDocumentKernel(kernel);
    if (kernel.modality === "conversational")
        return webPerceptFromConversationKernel(kernel);
 ...
 */
function webPerceptFromConversationKernel(kernel) {
    const lines = [];
    ...
    for (const signal of kernel.signals) {
        if (signal.type === "await-input")
            lines.push(signal.prompt);
    }
    const laid = layoutTextFrame({
        lines,
        affordances: kernel.affordances.flatMap((a) => {
            if (a.locator.kind !== "turn")
                return [];
 ...
 */
function webPerceptFromDocumentKernel(kernel) {
    const lines = [];
    const lineOfBlock = new Map();
    ...
    return {
        ...
        scrollY: kernel.totalBlocks > 0 ? Math.round((kernel.blocksRead / kernel.totalBlocks) * 1000) : 0,
        scrollHeight: 1000,
        screenshot: null,
        elements: laid.elements,
        dialogs: legacyDialogs(kernel.signals),
...
function legacyDialogs(signals) {
    return signals
        .filter((s) => s.type === "dialog" || s.type === "error" || s.type === "not-understood")
        .map((s) => ({ text: s.text, box: null }));
}
/** Kernel kinds that are not ARIA roles map onto the nearest legacy role. */
function legacyRole(kind, editable) {
    if (LEGACY_ROLES.has(kind))
        return kind;
    if (editable)
        return "textbox";
    if (kind === "mcp.field")
    ...
    if (kind === "mcp.tool")
        return "menuitem";
    if (kind === "chat.suggestion")
        return "button";
    if (kind === "chat.handoff")
        return "link";
    if (kind === "chat.turn")
        return "text";
    if (kind === "doc.reference")
        return "link";
    if (kind === "doc.section")
        return "menuitem";
    if (kind === "doc.figure")
        return "image";
    if (kind === "doc.table" || kind === "doc.metric")
        return "table";
    return "button";
}
function legacyLabel(description) {
...
//# sourceMappingURL=kernelView.js.map