...
function sizeConfidence(size) {
    return clamp01(0.4 + 0.5 * (Math.min(size, 40) / 40));
}
const SEVERITY_ORDER = { critical: 0, major: 1, minor: 2, info: 3 };
function worstSeverity(observations) {
    let worst = "info";
    for (const o of observations) {
        if (SEVERITY_ORDER[o.severity] < SEVERITY_ORDER[worst])
            worst = o.severity;
    }
    return worst;
}
function stanceFor(observations) {
...
function assemble(role, summary, observations, recommendations, size, confidenceBoost = 0) {
    return {
        role,
        summary,
...
function findingsByCategory(study, categories) {
...
export function uxResearcher(study) {
    const observations = [];
    ...
    const failing = study.segments.find((s) => s.key === "frustrated-quitters" ||
    ...
    return assemble("UX Researcher", `${pct(study.successRate)} task success across ${study.size} simulated users.`, observations, recommendations, study.size);
}
/** Accessibility Specialist — a11y/visual findings and at-risk personas. */
export function accessibilitySpecialist(study) {
    const observations = [];
    const recommendations = [];
    ...
    return assemble("Accessibility Specialist", a11y.length
        ? `${a11y.length} accessibility/visual issues recur population-wide.`
        : "No systemic accessibility issues detected.", observations, recommendations, study.size);
}
/** QA Engineer — reproducible broken affordances and error-recovery gaps. */
export function qaEngineer(study) {
    const observations = [];
    const recommendations = [];
    const broken = study.topFindings.filter((f) => f.category === "error-recovery" || /no visible response/i.test(f.title));
    ...
    return assemble("QA Engineer", broken.length
        ? `${broken.length} reproducible interaction defects.`
        : "No reproducible interaction defects.", observations, recommendations, study.size);
}
/** Interaction Designer — navigation efficiency and dead-ends. */
export function interactionDesigner(study) {
    const observations = [];
    const recommendations = [];
    ...
    return assemble("Interaction Designer", "Navigation efficiency and wayfinding assessment.", observations, recommendations, study.size);
}
/** Behavioral Psychologist — the emotional arc of the population. */
export function behavioralPsychologist(study) {
    const observations = [];
    const recommendations = [];
    observations.push({
    ...
    return assemble("Behavioral Psychologist", `Population ended at ${study.frustration.mean} frustration / ${study.trust.mean} trust.`, observations, recommendations, study.size);
}
/** Product Manager — where a fix buys the most completion. */
export function productManager(study) {
    const observations = [];
    const recommendations = [];
    ...
    return assemble("Product Manager", `${pct(study.successRate)} success — prioritizing the highest-leverage fix.`, observations, recommendations, study.size);
}
...
export function runSpecialists(study) {
