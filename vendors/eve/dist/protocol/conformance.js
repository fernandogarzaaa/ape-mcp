...
export function checkCorpus(corpus) {
    ...
    corpus.split("\n").forEach((line, index) => {
        ...
        try {
            ...
            fail(`could not be canonicalized: ${err.message}`);
        }
        // 2. Seal.
        try {
            ...
            fail(`seal could not be checked: ${err.message}`);
        }
        // 3. Structure.
        for (const detail of structuralProblems(document))
            fail(detail);
    });
 ...
 */
function provenanceEdgeProblems(corpus) {
    const failures = [];
    ...
    corpus.split("\n").forEach((line, index) => {
        ...
        const derived = Array.isArray(derivedFrom)
            ? derivedFrom.filter((v) => typeof v === "string")
            : [];
        rows.push({
            line: lineNumber,
            documentType: typeof document.type === "string" ? document.type : "?",
    ...
    for (const row of rows) {
        ...
        if (row.documentType !== "FitnessResult")
            continue;
        const document = docById.get(row.id);
        ...
        if (baselineRuns !== candidateRuns) {
            failures.push({
                line: row.line,
                documentType: row.documentType,
                detail: `baseline.runs (${baselineRuns}) and candidate.runs (${candidateRuns}) disagree; ` +
        ...
        // meaningful.
        if (baselineRuns === 0 && candidateRuns === 0)
            continue;
        ...
        const completions = row.derived
            ...
            .filter((doc) => doc?.type === "SimulationCompleted");
        if (completions.length === 0) {
            failures.push({
                line: row.line,
                documentType: row.documentType,
                detail: "provenance.derived_from names no SimulationCompleted; a measurement that cannot " +
        ...
        const satisfied = completions.some((run) => {
            ...
            return (run.subject_id === document.mutation_id &&
        ...
        if (!satisfied) {
            failures.push({
                line: row.line,
                documentType: row.documentType,
                detail: `${completions.length} referenced SimulationCompleted event(s) exist, but none has ` +
...
function structuralProblems(document) {
    const problems = [];
    if (document.cp !== "cp1")
        problems.push('`cp` must be the string "cp1"');
    ...
    if (typeof provenance !== "object" || provenance === null) {
        ...
        for (const member of REQUIRED_PROVENANCE_MEMBERS) {
            if (!(member in record))
                problems.push(`provenance is missing \`${member}\``);
        }
        if (!["adam", "eve", "axiom"].includes(record.authored_by)) {
 ...
 */
const SIGNED_BASIS_POINT_MEMBERS = new Set(["delta_bp"]);
/**
 ...
 */
function walkBasisPoints(value, path, problems) {
...
export function checkManifest(manifest, files) {
    ...
    for (const line of manifest.split("\n")) {
        ...
        if (actual !== expected) {
            failures.push(`${path}: manifest records ${expected.slice(0, 12)}…, vendored copy hashes ` +
                `${actual.slice(0, 12)}…  (re-vendor from the normative source in AXIOM-AETHER)`);
        }
    }
    if (matched === 0) {
        failures.push("no supplied file matched any manifest entry; the vendored paths have drifted from the manifest's");
    }
    return failures;
}
