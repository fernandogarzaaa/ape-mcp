    ...
    switch (typeof value) {
        ...
        case "number":
            if (!Number.isInteger(value)) {
                throw new CanonicalError(`CP/1 permits integers only, found ${value}; ratios cross the wire as basis points`, path);
            }
        ...
        case "object": {
            ...
            const members = keys.map((key) => `${JSON.stringify(key)}:${write(record[key], `${path}.${key}`)}`);
            return `{${members.join(",")}}`;
        }
        default:
...
function stripHash(document) {
    const provenance = document.provenance;
    if (typeof provenance !== "object" || provenance === null)
        return { ...document };
    const { content_hash: _omitted, ...rest } = provenance;
 ...
 * needed to refuse it.
 */
export function seal(document) {
    const provenance = document.provenance;
    if (typeof provenance !== "object" || provenance === null) {
        throw new CanonicalError("a CP/1 document must carry a `provenance` object before it can be sealed", "$.provenance");
    }
    const hash = contentHash(document);
    return {
        ...document,
        provenance: { ...provenance, content_hash: hash },
    };
}
 ...
 */
export function toBasisPoints(ratio) {
...
export function toSignedBasisPoints(ratio) {
    if (!Number.isFinite(ratio))
        return 0;
