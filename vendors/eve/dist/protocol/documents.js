...
function outcomeOf(iteration) {
    if (iteration.action.kind === "abandon")
        return "abandoned";
    ...
    if (outcome.errorPerceived)
        return "error";
    if (!outcome.screenChanged && iteration.prediction.expectsChange)
        return "no_change";
 ...
 */
function actorFor(kind, requested) {
    const permitted = EVENT_EMITTER[kind];
    if (requested === undefined) {
        if (permitted.length !== 1) {
            throw new Error(`${kind} may be emitted by ${permitted.join(" or ")}; actor must be given explicitly`);
        }
        return permitted[0];
    }
