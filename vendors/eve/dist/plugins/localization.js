...
export class LocalizationPlugin {
    ...
    async onPercept(ctx, percept) {
        ...
        if (this.reportedScreens.has(percept.url))
            return;
        ...
        if (culture.readingDirection === "rtl" && !this.rtlReported) {
            const leftAnchored = percept.elements.filter((e) => e.interactive && e.box.x < percept.viewport.width * 0.15).length;
            const rightAnchored = percept.elements.filter((e) => e.interactive && e.box.x > percept.viewport.width * 0.6).length;
