...
export class PerformancePlugin {
    name = "performance";
    latencies = [];
    slowReported = new Set();
    async onOutcome(ctx, outcome, percept) {
    ...
    async onSessionEnd(ctx) {
        ...
        if (p90 > 2000) {
            ctx.report({
                ...
                description: "Across the whole session, one in ten interactions kept the operator waiting more than two seconds — the product feels slow even if no single wait is dramatic.",
                evidence: [
