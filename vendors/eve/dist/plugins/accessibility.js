...
export class AccessibilityPlugin {
    ...
    async onPercept(ctx, percept) {
        ...
        // visual audit.
        if (!ctx.capabilities.spatial)
            return;
        // Images with no textual alternative.
        const unlabeledImages = percept.elements.filter((el) => el.role === "image" && !el.text.trim() && el.box.width > 32 && el.box.height > 32);
        if (unlabeledImages.length > 0) {
            ctx.report({
                severity: "minor",
                category: "accessibility",
                title: `${unlabeledImages.length} significant image(s) carry no textual alternative`,
                description: "Meaningful images without alt text are invisible to screen-reader users and convey nothing when they fail to load.",
        ...
        const unlabeledControls = percept.elements.filter((el) => el.interactive &&
            !el.disabled &&
            !el.text.trim() &&
        ...
        if (unlabeledControls.length > 0) {
            ctx.report({
                ...
                description: "Controls without visible or accessible labels cannot be understood by assistive technology, and sighted users must guess their purpose from position alone.",
        ...
        if (ctx.capabilities.pointer === "touch") {
            const smallTargets = percept.elements.filter((el) => el.interactive &&
                !el.disabled &&
                el.box.width > 0 &&
                el.box.height > 0 &&
            ...
            if (smallTargets.length > 0) {
                ctx.report({
                    ...
                    description: "Interactive controls below the widely-used 44×44 CSS px minimum tap-target size are hard to hit accurately with a finger, especially for users with reduced motor precision.",
                    evidence: smallTargets
                        .slice(0, 3)
                        .map((el) => `${el.role} "${el.text.slice(0, 30)}" is ${Math.round(el.box.width)}×${Math.round(el.box.height)}px`),
                    url: percept.url,
            ...
            const occluded = percept.elements.filter((el) => el.occludedByKeyboard && (el.interactive || el.text.trim()));
            if (occluded.length > 0) {
                ctx.report({
                    severity: "major",
                    category: "accessibility",
                    title: `${occluded.length} element(s) are covered by the on-screen keyboard`,
                    description: "While a text field holds focus, the soft keyboard covers the bottom of the screen. Content placed there — often a submit button or a validation message — is invisible until the keyboard is dismissed.",
                    evidence: occluded
                        .slice(0, 3)
                        .map((el) => `${el.role} "${el.text.slice(0, 30)}" at y=${Math.round(el.box.y)}`),
                    url: percept.url,
                    recommendation: "Keep primary actions and validation messages above the keyboard's typical coverage, or scroll them into view on focus.",
                });
            }
        }
        ...
        if (ctx.persona.accessibility.keyboardOnly) {
            ...
            if (!anyFocus && interactive.length > 3) {
                ctx.report({
                    ...
                    description: "A keyboard-only user cannot tell where they are: no element on this screen shows focus, so Tab navigation is blind.",
                    evidence: [
     ...
     */
    async onSessionEnd(ctx, iterations) {
        if (ctx.capabilities.canHover)
            return;
        const reported = new Set();
        for (const it of iterations) {
            if (it.action.kind !== "hover")
                continue;
            ...
            ctx.report({
                ...
                description: "The operator expected hovering to reveal or activate this control, but this surface has no persistent pointer — hover-gated content is invisible to a touch user, not merely awkward to reach.",
                evidence: [
                    `Attempted hover over ${target.role} at (${Math.round(target.box.x)}, ${Math.round(target.box.y)}) on ${it.url}`,
                ],
                url: it.url,
                recommendation: "Expose this affordance through a tap-visible control (icon, always-visible label, or tap-to-reveal) instead of relying on :hover.",
            });
        }
    }
}
