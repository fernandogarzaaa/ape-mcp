...
export function overlapRatio(question, reply) {
 ...
 */
export function isNearMiss(question, reply) {
    const asked = contentWords(question);
    if (asked.size < 2)
        return false;
    if (reply.split(/\s+/).filter(Boolean).length < MIN_JUDGEABLE_REPLY_WORDS)
        return false;
    if (contentWords(reply).size < MIN_JUDGEABLE_REPLY_STEMS)
        return false;
