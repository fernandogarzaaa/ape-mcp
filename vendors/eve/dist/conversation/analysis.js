...
export function analyzeConversation(input) {
    ...
    const everOfferedHandoff = surfaceTurns.some((turn) => offersHandoff({ text: turn.text, affordances: affordancesOf(turn) }));
    ...
    const understanding = clamp01(1 - (admittedMisses * 0.6 + silentMisses) / asked - input.repairAttempts * 0.1);
    ...
    const recovery = failures === 0 && input.repairAttempts === 0
        ? 1
        : clamp01((everOfferedHandoff ? 0.5 : 0) +
...
function collectFindings(input, observed) {
    const findings = [];
    ...
    if (input.repairAttempts > REPAIR_TOLERANCE) {
        findings.push({
            ...
            description: "Every rephrase is the person doing the work the surface failed to do, and each one costs more patience than the last. Past two attempts most people stop believing the next phrasing will land and leave — often for a channel that costs far more to serve.",
            evidence: repairEvidence(turns),
            url,
            recommendation: "Look at what they actually typed. If a real intent is missing, add it; if it exists but is not matching, the routing is the bug — not the phrasing.",
        });
    }
    ...
    if (observed.silentMisses > 0) {
        findings.push({
            ...
            description: "This is worse than admitting the miss. A confident answer to a nearby question reads as an answer, so the person acts on it, or spends a turn working out that it was not one. Either way they trust the next reply less.",
            evidence: silentMissEvidence(turns),
            url,
            recommendation: 'Say when confidence is low. "I\'m not sure I follow — did you mean X?" costs one turn; a confident near-miss costs the conversation.',
        });
    }
    /* ---- no way to a person -------------------------------------------- */
    ...
    if (struggled && !observed.everOfferedHandoff) {
        findings.push({
            ...
            description: "The surface did not understand and never offered a way out. A person who cannot get through and cannot escape does not try harder — they leave, and the cost of the contact lands somewhere more expensive anyway.",
            evidence: [
            ...
            url,
            recommendation: "Offer a person after the second failed attempt, unprompted. Waiting for someone to ask assumes they will, and by then they have gone.",
        });
    }
    ...
    if (amnesia.length > 0) {
        findings.push({
            ...
            description: "Being asked to repeat yourself is the clearest possible signal that nothing is listening. It converts a conversation into a form, and a badly designed one.",
            evidence: amnesia,
            url,
            recommendation: "Carry what has been said forward, and reflect it back rather than re-asking.",
        });
    }
    /* ---- dead air ------------------------------------------------------- */
    if (observed.maxLatencyMs !== null && observed.maxLatencyMs > DEAD_AIR_MS) {
        ...
        findings.push({
            ...
            description: "In a medium that looks like talking to someone, silence reads as being ignored rather than as work in progress. Without a visible sign it is still going, people re-send, refresh, or leave.",
            evidence: slow
                .slice(0, 3)
                .map((turn) => `${Math.round((turn.latencyMs ?? 0) / 100) / 10}s before: ${truncate(turn.text, 70)}`),
            url,
            recommendation: "Show that it is working, and say what it is doing. A visible 'checking your orders…' buys far more patience than a spinner.",
        });
    }
    ...
    if (overlong.length > 0) {
        findings.push({
            ...
            description: "A chat window is read at the pace of speech, not of a document. Past a screenful people skim for the one line that matters and miss it, then ask again.",
            evidence: overlong
                .slice(0, 3)
    ...
    if (input.kind === "support" && turns.length > SUPPORT_TURN_BUDGET && !input.goalAchieved) {
        findings.push({
            ...
            description: "A support conversation is judged on how fast it routes, not on how long it can keep going. Every extra turn is a person deciding whether this is still worth their time.",
            evidence: [`${turns.length} turns, goal ${input.goalAchieved ? "achieved" : "not achieved"}`],
            url,
            recommendation: "Route to the right answer or the right person within a few turns; there is no prize for staying in the conversation.",
        });
    }
    return findings;
}
...
/* ------------------------------------------------------------------ */
const ADMITTED_MISS = /\b(?:didn'?t|did not|don'?t|do not)\s+(?:quite\s+)?(?:understand|catch|get|follow)\b|\b(?:could|can) you (?:please )?(?:rephrase|clarify|try again)\b|\bnot sure i (?:understand|follow)\b/i;
/** Phrasings that ask for something, used to spot a re-ask. */
const ASKS_FOR = /\b(?:what(?:'s| is)? your|can you (?:give|provide|confirm|tell me)|please (?:provide|share|confirm|enter)|could you (?:give|provide|confirm))\b[^.?!]{0,60}/gi;
function admittedMiss(text) {
 ...
 */
function countSilentMisses(turns) {
    let count = 0;
    for (let i = 1; i < turns.length; i++) {
        ...
        if (!reply || !asked)
            continue;
        if (reply.speaker !== "surface" || asked.speaker !== "operator")
            continue;
        if (saidItDidNotUnderstand(reply))
            continue;
...
function silentMissEvidence(turns) {
    const evidence = [];
...
function amnesiaCount(turns) {
    return amnesiaEvidence(turns).length;
}
function amnesiaEvidence(turns) {
    const evidence = [];
    const saidSoFar = [];
