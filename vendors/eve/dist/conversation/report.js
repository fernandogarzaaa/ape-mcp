 ...
 */
export function renderConversationMarkdown(analysis, turns) {
    const lines = [];
    lines.push(`# Conversation report — ${analysis.address}`);
    lines.push("");
    lines.push(`**${analysis.persona}** talked to a ${analysis.kind} surface for ${analysis.turnCount} turn(s), asking ${analysis.operatorTurns} time(s).`);
    lines.push("");
    lines.push("| | |");
    lines.push("| --- | --- |");
    lines.push(`| Understood the person | \`${bar(analysis.understanding / 100, 12)}\` **${analysis.understanding}/100** |`);
    lines.push(`| Showed it understood | \`${bar(analysis.grounding / 100, 12)}\` **${analysis.grounding}/100** |`);
    lines.push(`| Recovered when it failed | \`${bar(analysis.recovery / 100, 12)}\` **${analysis.recovery}/100** |`);
    lines.push(`| Had to rephrase | ${analysis.repairAttempts}× |`);
    lines.push(`| Missed the question | ${analysis.admittedMisses} admitted, **${analysis.silentMisses} silently** |`);
    lines.push(`| Offered a person | ${analysis.everOfferedHandoff ? "yes" : "**never**"} |`);
    if (analysis.meanLatencyMs !== null) {
        lines.push(`| Reply time | mean ${(analysis.meanLatencyMs / 1000).toFixed(1)}s, slowest ${((analysis.maxLatencyMs ?? 0) / 1000).toFixed(1)}s |`);
    }
    lines.push("");
    /* ---- the transcript ---- */
    ...
    for (const turn of turns) {
        ...
        const timing = turn.latencyMs !== undefined && turn.latencyMs >= 1000
