...
const CONVERSATIONAL_ONLY = ["conversational"];
...
const DIMENSION_DESCRIPTIONS = {
    "conversation.understanding": "Whether the surface understood what was asked — first time, and after the person tried again.",
    "conversation.grounding": "Whether it showed it understood and carried the conversation forward: no confident near-misses, no asking twice for the same thing.",
    "conversation.recovery": "What happened when it failed — whether it said so, and whether there was a way through to a person.",
    "conversation.responsiveness": "Whether replies arrived before silence started reading as being ignored.",
};
...
export function registerConversationVocabulary() {
    if (registered)
        return;
    registered = true;
    ...
    for (const verb of CONVERSATION_VERBS) {
        if (!verb.startsWith("chat.") || actionVerbRegistry.has(verb))
            continue;
        registerActionVerb({
            id: verb,
