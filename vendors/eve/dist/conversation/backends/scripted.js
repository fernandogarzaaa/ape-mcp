...
export class ScriptedBackend {
    ...
    async send(message) {
        ...
        const rule = this.script.rules[index];
        if (rule.once)
            this.used.add(index);
        return {
            text: rule.reply,
...
function matches(when, message) {
    if (typeof when === "string")
        return message.toLowerCase().includes(when.toLowerCase());
...
export const DEMO_SUPPORT_BOT = {
    ...
    rules: [
        ...
        },
        {
            ...
            reply: "You can track your order from the Orders page in your account. Just sign in and select the order you'd like to track.",
            affordances: [{ id: "s1", kind: "suggestion", label: "Go to Orders" }],
        },
        {
            when: /\brefund\b|\bmoney back\b|\breturn\b/i,
            reply: "I can help with returns! Most items can be returned within 30 days. Would you like me to start a return?",
        ...
        },
        {
            ...
            reply: "Our billing cycle runs monthly, and invoices are issued on the first of each month. You can view all invoices under Billing in your account settings.",
        },
        {
            when: /\bcancel\b.*\bsubscription\b|\bunsubscribe\b/i,
            reply: "I'm sorry to hear that! Before you go — did you know you can pause your subscription instead? Pausing keeps your history and settings.",
        },
        {
    ...
    fallback: "I want to make sure I get this right. You can find answers to most questions in our Help Centre, which covers orders, returns, billing and account settings.",
};
