...
export function renderProductIntelligenceMarkdown(intel) {
    const lines = [
    ...
    for (const p of intel.personas) {
        lines.push(`- **${p.archetype}** — ${pct(p.share)} of users (typically \`${p.typicalPersona}\`), ` +
    ...
    for (const f of intel.featureImportance) {
        lines.push(`- **${f.feature}** — importance ${f.importance}/100 (reach ${pct(f.reach)}, ${f.visits} visits)${f.onCriticalPath ? " · on the critical path" : ""}`);
    }
