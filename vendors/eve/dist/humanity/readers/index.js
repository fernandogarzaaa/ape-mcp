 ...
 */
import { detectDelimited, detectJson, detectYaml, readDelimited, readJson, readYaml, } from "./data.js";
...
const READERS = [
    {
        ...
        format: "markdown",
        detect: (input) => {
            if (input.extension && MARKDOWN_EXTENSIONS.has(input.extension))
                return 0.95;
...
export function selectReader(input) {
    let best = READERS[READERS.length - 1];
    let bestScore = -1;
 ...
 */
export function readArtifactText(input) {
    ...
    const effective = input.format === "slides" ? { ...input, genre: input.genre ?? "presentation" } : input;
