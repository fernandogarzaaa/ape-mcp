 ...
 */
const PROMPT = /^(?:[\w.-]+@[^$#>❯➜]*|PS [A-Za-z]:[^$#>❯➜]*|~[^$#>❯➜]*|[\w.-]*[ \t]*)?(?:\$|#|>|❯|➜)[ \t]+(\S.*)$/;
const ERROR_LINE = /\b(?:error|fatal|exception|traceback|panic|failed|failure|cannot|denied|refused|unable to|not found)\b/i;
...
export function detectTranscript(input) {
    ...
    const logs = lines.filter((l) => LOG_LEVEL.test(stripAnsi(l)) || TIMESTAMP.test(stripAnsi(l))).length;
...
export function readTranscript(input) {
    ...
    const flush = () => {
        ...
        output = [];
        if (text)
            builder.add({ kind: outputKind, text });
        outputKind = "output";
    };
    for (const line of lines) {
        ...
        if (ERROR_LINE.test(line) && !WARNING_LINE.test(line))
            outputKind = "error";
        output.push(line);
    }
    flush();
    if (!sawCommand)
        builder.setMeta("shape", "log");
