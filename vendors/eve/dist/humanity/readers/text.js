...
export function readText(input) {
    ...
    const genre = input.genre ?? inferGenre(lines);
...
function isHeadingLike(text) {
    ...
    if (words.length === 0 || words.length > MAX_HEADING_WORDS)
        return false;
    if (/[.!?,;]$/.test(text))
        return false;
...
function inferGenre(lines) {
    ...
    if (entries >= 3 && (usage || entries / nonEmpty.length >= 0.4))
        return "interface";
    const metrics = nonEmpty.filter((l) => parseMetric(l.trim()) !== null).length;
    if (metrics >= 4 && metrics / nonEmpty.length >= 0.3)
        return "analytics";
    return "document";
}
