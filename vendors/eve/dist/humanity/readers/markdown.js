...
export function looksLikeSlides(text, extension) {
    if (extension === ".slides.md")
        return true;
    const lines = text.split(/\r?\n/);
    let breaks = 0;
    let headingsAfterBreak = 0;
    for (let i = 0; i < lines.length; i++) {
        if (!THEMATIC_BREAK.test(lines[i] ?? ""))
            continue;
        // A `---` directly under text is a setext H2 underline, not a break.
        if ((lines[i - 1] ?? "").trim() && SETEXT_H2.test(lines[i] ?? ""))
            continue;
        breaks++;
...
export function readMarkdown(input) {
    ...
    const genre = input.genre ?? (slides ? "presentation" : inferProseGenre(body));
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (THEMATIC_BREAK.test(line)) {
            ...
            if (slides)
                ...
                builder.add({ kind: "separator", text: "———" });
            continue;
        }
        ...
        if (heading) {
            ...
            if (!slides && depth <= 2)
                builder.startSection(text);
            else if (slides && builder.size === 0)
                builder.startSection(text);
            else if (slides)
                builder.nameCurrentSection(text);
 ...
 */
function emitProse(builder, text, kind = "paragraph", depth = 0) {
    ...
    if (figures.length > 0) {
        ...
        if (remainder)
            builder.add({ kind, text: remainder, depth });
        return;
    }
...
export function stripInline(text) {
    return stripHtmlTags(text
        .replace(IMAGE, (_m, alt) => (alt ? `${alt} (image)` : "(image)"))
        .replace(LINK, (_m, label) => label)
        .replace(/`([^`]+)`/g, "$1")
        .replace(/\*\*([^*]+)\*\*/g, "$1")
        .replace(/(^|[^*])\*([^*]+)\*/g, "$1$2")
        .replace(/__([^_]+)__/g, "$1"))
...
function readTable(lines, start) {
    ...
    if (!header)
        return null;
    if (!isTableDivider(lines[start + 1] ?? ""))
        return null;
...
export function tableSummary(detail) {
