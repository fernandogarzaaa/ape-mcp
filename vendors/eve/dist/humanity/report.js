 ...
 */
export function renderComprehensionMarkdown(analysis, artifact) {
    const lines = [];
    const blocksById = new Map(artifact.blocks.map((block) => [block.id, block]));
    lines.push(`# Reading report — ${artifact.title}`);
    lines.push("");
    lines.push(`**${analysis.persona}** read \`${artifact.address}\` — a ${artifact.genre} of ${artifact.sections.length} ${artifact.sections[0]?.noun ?? "section"}(s), ${artifactWordCount(artifact)} words.`);
    lines.push("");
    lines.push("| | |");
    lines.push("| --- | --- |");
    ...
    lines.push(`| Reading ease | Flesch ${analysis.readability.fleschReadingEase} (grade ${analysis.readability.gradeLevel}) |`);
    lines.push(`| Sentences | ${analysis.readability.sentences}, mean ${analysis.readability.meanSentenceWords} words, longest ${analysis.readability.longestSentenceWords} |`);
    lines.push("");
    ...
    if (lost.length === 0) {
        ...
        for (const block of lost) {
            ...
            lines.push(`- \`${bar(block.comprehension, 12)}\` **${Math.round(block.comprehension * 100)}%** — ${section?.noun ?? "section"} "${section?.title ?? "?"}"`);
    ...
    for (const section of artifact.sections) {
        ...
        lines.push(`| ${section.index + 1}. ${section.title} | ${words} | \`${bar(understood, 10)}\` ${Math.round(understood * 100)}% |`);
    }
    lines.push("");
    /* ---- findings ---- */
