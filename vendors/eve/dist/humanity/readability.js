...
export function measureReadability(text) {
    ...
    return {
        ...
        longestSentenceWords: Math.max(...sentences.map((sentence) => (sentence.match(WORD) ?? []).length)),
    };
}
 ...
 * never checked.
 */
export function findAcronyms(passages) {
...
export function findJargon(text) {
    ...
    for (const word of words) {
        ...
        // Common inflections of the same terms ("sharding", "refactored").
            ...
            if (stem.length >= 5 && JARGON.has(stem))
                found.add(stem);
        }
    }
    return [...found];
...
export function jargonVocabulary() {
    return [...JARGON];
}
