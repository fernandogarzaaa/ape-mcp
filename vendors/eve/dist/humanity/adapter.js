...
export class HumanityAdapter {
    ...
    constructor(options = {}) {
        ...
        if (options.artifact)
            this.openArtifact(options.artifact);
    }
    ...
    async typeText(_text, _perCharIntervalMs) {
    ...
    requireArtifact() {
        if (!this.artifact)
            throw new Error("HumanityAdapter: open() an artifact before reading it");
        return this.artifact;
    }
    recomputeComprehension() {
        const artifact = this.requireArtifact();
    ...
    stateFor(section) {
        ...
        if (existing)
            return existing;
        const created = { read: new Set(), skimmed: false, rereads: 0 };
    ...
    goTo(section, options = {}) {
        if (options.record !== false)
            this.trail.push(this.section);
    ...
    atEnd() {
        const artifact = this.requireArtifact();
        if (this.section !== artifact.sections.length - 1)
            return false;
    ...
    gapsFor(blocks) {
        ...
        for (const block of blocks) {
            const understanding = this.comprehension.get(block.id);
            if (!understanding || understanding.comprehension >= 0.5)
                continue;
    ...
    resolveReference(reference) {
        ...
        if (!anchor || /^[a-z]+:/i.test(reference))
            return null;
        const index = artifact.sections.findIndex((section) => section.title
            .toLowerCase()
            .replace(/[^a-z0-9 ]/g, "")
            .trim() === anchor);
 ...
 */
function endMarkerFor(artifact) {
...
function toContentBlock(block) {
    ...
    if (block.metric)
        detail.metric = block.metric;
    if (block.figure)
        detail.figure = block.figure;
    if (block.reference)
        detail.reference = block.reference;
    if (block.language)
        detail.language = block.language;
    return {
        id: block.id,
        kind: block.kind,
        text: block.text,
...
export function humanityAdapterFor(address, text, options = {}) {
    const adapter = new HumanityAdapter(options);
    adapter.openArtifact(artifactFromText(address, text, {
