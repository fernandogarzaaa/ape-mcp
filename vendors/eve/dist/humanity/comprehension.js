...
import { findAcronyms, findJargon, measureReadability, splitSentences, } from "./readability.js";
 ...
 */
function domainTolerance(persona) {
...
export function comprehendBlock(block, persona, context) {
    ...
    if (block.kind !== "code" && block.kind !== "table") {
        for (const sentence of splitSentences(block.text)) {
            ...
            if (length > LONG_SENTENCE_WORDS) {
                obstacles.push({
                    kind: "long-sentence",
                    cost: clamp01((length - LONG_SENTENCE_WORDS) / 40) * (1 - persona.traits.thoroughness * 0.4),
    ...
    for (const acronym of collectAcronyms(block.text)) {
        if (!context.undefinedAcronyms.has(acronym))
            continue;
        obstacles.push({
            kind: "undefined-term",
    ...
    const registerCost = readability.words >= 25 && readability.fleschReadingEase < HARD_PROSE_FLESCH
    ...
    for (const obstacle of obstacles)
        comprehension *= 1 - clamp01(obstacle.cost);
    const effort = clamp01(words / 200 + obstacles.reduce((total, o) => total + o.cost, 0) * 0.4 + registerCost * 0.3);
    return {
        blockId: block.id,
 ...
 */
export function analyzeComprehension(artifact, persona) {
    const passages = artifact.blocks.map((block) => block.text);
    const acronyms = findAcronyms(passages);
    const undefinedAcronyms = new Set(acronyms.filter((use) => !use.introduced).map((use) => use.acronym));
    const blocks = artifact.blocks.map((block) => comprehendBlock(block, persona, { undefinedAcronyms, genre: artifact.genre }));
...
function collectFindings(artifact, persona, blocks, acronyms, readability) {
    const findings = [];
    ...
    if (undefinedAcronyms.length > 0) {
        findings.push({
            ...
            description: "The artifact assumes the reader already knows these. A reader who does not has no way to recover the meaning from the text, and every later sentence that depends on the term is lost with it.",
            evidence: undefinedAcronyms
                .slice(0, 5)
                .map((use) => `"${use.acronym}" first appears in: ${truncate(use.firstSeenIn)}`),
            url,
            recommendation: 'Expand each term on first use — "Service Level Objective (SLO)" — or drop it for a phrase the audience already has.',
        });
    }
    /* ---- register ------------------------------------------------------ */
    if (readability.words >= 100 && readability.fleschReadingEase < HARD_PROSE_FLESCH) {
        findings.push({
            ...
            description: "Sentences are long and words are heavy enough that the reader has to hold each clause open while parsing the next. That cost is paid on every sentence, so it compounds across the whole artifact.",
            evidence: [
            ...
            url,
            recommendation: "Split sentences past ~25 words and replace multi-syllable abstractions with the concrete thing they stand for.",
        });
    }
    /* ---- structure ----------------------------------------------------- */
    const headings = artifact.blocks.filter((block) => block.kind === "heading" || block.kind === "title");
    ...
    if (totalWords > UNSTRUCTURED_WORDS && headings.length === 0) {
        findings.push({
            ...
            description: "There is no way to scan this, no way to find one part again, and no way back in after an interruption. Readers who cannot scan do not read more slowly — they stop.",
            evidence: [
    ...
    /* ---- walls of text -------------------------------------------------- */
    const walls = blocks.filter((block) => block.obstacles.some((obstacle) => obstacle.kind === "wall-of-text"));
    if (walls.length > 0) {
        findings.push({
            severity: walls.length >= 3 ? "major" : "minor",
            category: "humanity.structure",
            title: `${walls.length} paragraph(s) run past ${WALL_OF_TEXT_WORDS} words`,
            description: "In an unbroken block the eye loses its place on line return, and re-finding it costs the reader the sentence they were holding.",
            evidence: walls
                .slice(0, 3)
                .map((block) => truncate(byId.get(block.blockId)?.text ?? block.blockId)),
            url,
            recommendation: "Break each into paragraphs of one idea, or lift the list that is hiding inside it.",
        });
    }
    /* ---- figures -------------------------------------------------------- */
    const unlabeledFigures = artifact.blocks.filter((block) => block.figure && !block.figure.alt && !block.figure.caption);
    if (unlabeledFigures.length > 0) {
        findings.push({
            severity: "major",
            category: "humanity.comprehension",
            title: `${unlabeledFigures.length} figure(s) carry no caption or alternative text`,
            description: "A chart with no caption asserts nothing: the reader has to infer the claim from the picture, and a reader using a screen reader gets no claim at all.",
            evidence: unlabeledFigures
                .slice(0, 4)
                .map((block) => `figure ${block.figure?.source ?? "(inline)"} in ${sectionName(artifact, block)}`),
            url,
            recommendation: 'Caption every figure with the takeaway, not the subject: "Signups fell 12% after the March release", not "Signup chart".',
        });
    }
    /* ---- genre expectations --------------------------------------------- */
 ...
 */
function genreFindings(artifact, persona, blocks, byId) {
    const findings = [];
    const url = artifact.address;
    switch (artifact.genre) {
        case "presentation": {
            const dense = artifact.sections.filter((section) => {
                const words = section.blocks.reduce((total, index) => total + wordCount(artifact.blocks[index]?.text ?? ""), 0);
                const bullets = section.blocks.filter((index) => artifact.blocks[index]?.kind === "list-item").length;
            ...
            if (dense.length > 0) {
                findings.push({
                    ...
                    description: "An audience given more than about sixty words reads the slide instead of listening to the speaker, and finishes doing neither.",
            ...
            if (labelTitles.length >= Math.max(2, artifact.sections.length / 2)) {
                findings.push({
                    ...
                    description: 'A title like "Results" tells the reader where they are but not what to conclude. Skimmed later — which is how most decks are read — the deck says nothing.',
                    evidence: labelTitles.slice(0, 5).map((section) => `"${section.title}"`),
                    url,
                    recommendation: 'Make each title the sentence you want remembered: "Retention recovered in Q3" rather than "Retention".',
                });
            }
            break;
        }
        case "analytics": {
            const nakedMetrics = artifact.blocks.filter((block) => block.metric && !block.metric.baseline);
            if (nakedMetrics.length > 0) {
                findings.push({
                    ...
                    description: "A figure with no baseline cannot be judged good or bad, so the reader either invents a comparison or ignores the number. Both outcomes are worse than not reporting it.",
                    evidence: nakedMetrics
                        .slice(0, 5)
                        .map((block) => `${block.metric?.label}: ${block.metric?.value}`),
                    url,
                    recommendation: "Pair every number with its comparison — previous period, target, or the same number for a peer.",
                });
            }
            ...
            for (const block of artifact.blocks) {
                if (!block.table)
                    continue;
                const bare = block.table.columns.filter((column, index) => !columnDeclaresUnit(column) && columnIsNumeric(block.table, index));
                if (bare.length === 0)
                    continue;
                findings.push({
                    ...
                    description: "The reader has to infer the unit from the magnitude, which is a guess — and a guess about whether a latency figure is milliseconds or seconds is the difference between fine and an outage.",
                    ...
                    url,
                    recommendation: 'Put the unit in the header — "revenue (USD)", "p99 (ms)", "error rate (%)".',
                });
            }
            const unitless = artifact.blocks.filter((block) => block.metric && !block.metric.unit && !/[%$£€¥]/.test(block.metric.value));
            if (unitless.length >= 3) {
                findings.push({
                    severity: "minor",
                    category: "humanity.comprehension",
                    title: `${unitless.length} number(s) carry no unit`,
                    description: "The reader has to guess whether 4200 is dollars, users, milliseconds or requests — and a guess that lands wrong is worse than a blank.",
                    evidence: unitless
                        .slice(0, 5)
                        .map((block) => `${block.metric?.label}: ${block.metric?.value}`),
                    url,
        ...
        case "transcript": {
            ...
            if (withoutRemedy.length > 0) {
                findings.push({
                    ...
                    description: "This is the moment the reader is most stuck and least patient. An error that names a cause without a next step leaves them searching the web instead of fixing the problem.",
                    evidence: withoutRemedy.slice(0, 4).map((block) => truncate(block.text)),
                    url,
                    recommendation: "Follow each failure with the next action — the flag to pass, the file to edit, the command to run.",
                });
            }
            break;
        }
        case "data": {
            ...
            if (deepest > DEEP_NESTING) {
                findings.push({
                    ...
                    description: "Past about five levels the reader can no longer say which object a field belongs to without scrolling back, so every field is read twice.",
        ...
        case "interface": {
            const undocumented = artifact.blocks.filter((block) => block.kind === "field" && !/—|:|\s{2,}/.test(block.text));
            if (undocumented.length >= 3) {
                findings.push({
                    severity: "minor",
                    category: "humanity.comprehension",
                    title: `${undocumented.length} option(s) listed with no explanation`,
                    description: "A flag with no description forces the reader to try it and see, which is the experiment they came to this text to avoid.",
            ...
            if (conclusion !== null && artifact.blocks.length >= 8) {
                ...
                if (position > 0.75) {
                    findings.push({
                        ...
                        description: "Readers decide whether to continue in the first screen. A recommendation held back to the end reaches only the readers who never needed convincing.",
                        evidence: [
                            `conclusion at block ${conclusion + 1} of ${artifact.blocks.length}: ${truncate(artifact.blocks[conclusion]?.text ?? "")}`,
                        ],
                        url,
    ...
    if (artifact.genre === "document" || artifact.genre === "presentation") {
        ...
        if (totalWords > 120 && !tail.some((block) => hasCallToAction(block.text))) {
            findings.push({
                ...
                description: "The reader finishes, agrees, and does nothing — because nothing was asked of them. Comprehension without a next step is where most internal writing quietly fails.",
                evidence: tail.slice(-2).map((block) => truncate(block.text)),
                url,
                recommendation: "Close with the decision, the owner and the date, or the single next action.",
            });
        }
    }
    ...
    if (lost > 0 && blocks.length > 0 && lost / blocks.length >= 0.2) {
        findings.push({
            ...
            description: "Comprehension is compounding: each block a reader does not follow makes the next one harder, because the terms it introduced were the ones the next block assumed.",
            evidence: worst.map((block) => `${Math.round(block.comprehension * 100)}% — ${truncate(byId.get(block.blockId)?.text ?? "")}` +
                (block.obstacles.length > 0 ? ` [${block.obstacles.map((o) => o.kind).join(", ")}]` : "")),
            url,
            recommendation: "Start with the blocks listed above: they are where this reader stopped following.",
        });
    }
    return findings;
}
...
/* Helpers                                                             */
/* ------------------------------------------------------------------ */
const CONCLUSION = /\b(?:in conclusion|to conclude|we recommend|recommendation|therefore|in summary|takeaway|bottom line|the fix|next steps?)\b/i;
const CALL_TO_ACTION = /\b(?:next steps?|action items?|we recommend|please|you (?:should|can|must|need to)|to get started|run |install |contact |sign up|apply|decide|approve|review by|owner:|due )\b/i;
const REMEDY = /\b(?:try|run|install|set|add|remove|check|see|use|pass|update|upgrade|retry|instead|hint|suggestion|did you mean|to fix|help)\b/i;
/** A title that names a topic rather than asserting anything about it. */
const LABEL_TITLE = /^(?:[\w&/-]+(?:\s+[\w&/-]+){0,3})$/;
const ASSERTIVE = /\b(?:is|are|was|were|will|has|have|grew|fell|rose|dropped|beats|needs|should|must|why|how|we|our)\b/i;
function isLabelTitle(title) {
    const text = title.trim();
    if (!text || !LABEL_TITLE.test(text))
        return false;
...
function suggestsRemedy(text) {
    return REMEDY.test(text);
}
function findConclusionIndex(artifact) {
    for (let i = 0; i < artifact.blocks.length; i++) {
        if (CONCLUSION.test(artifact.blocks[i]?.text ?? ""))
            return i;
    }
    return null;
}
/** A header that names its unit: "(ms)", "revenue_usd", "latency ms", "% churn". */
function columnDeclaresUnit(column) {
    return /\(|%|\$|£|€|¥|\b(?:ms|s|sec|secs|min|hrs?|usd|eur|gbp|pct|percent|bytes|kb|mb|gb|tb|count|per\b)\b|_(?:ms|s|usd|eur|gbp|pct|kb|mb|gb)$/i.test(column);
}
/** A column is numeric when the rows that have a value are all numbers. */
function columnIsNumeric(table, index) {
...
function collectAcronyms(text) {
    return [...new Set(text.match(/\b[A-Z]{2,6}\b/g) ?? [])];
}
function sectionName(artifact, block) {
