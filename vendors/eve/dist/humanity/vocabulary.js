...
const DIMENSION_DESCRIPTIONS = {
    "humanity.comprehension": "Whether the reader actually understood it: terms defined before use, figures that assert something, numbers with a baseline, an ending that says what to do.",
    "humanity.readability": "The cost of parsing the prose itself — sentence length, syllable weight, and the register the writing assumes.",
    "humanity.structure": "Whether the artifact can be scanned, re-entered and navigated: headings, paragraph size, slide density, nesting depth.",
};
/**
...
export function registerHumanityVocabulary() {
    if (registered)
        return;
    registered = true;
    ...
    for (const verb of DOCUMENT_VERBS) {
        ...
        if (!verb.startsWith("doc.") || actionVerbRegistry.has(verb))
            continue;
        registerActionVerb({
            id: verb,
