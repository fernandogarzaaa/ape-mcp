 ...
 */
export const TRAIT_PROJECTIONS = {
 ...
 */
const RETIRE_SKILL_DELTA = { trait: "techLiteracy", amount: -0.1 };
/**
...
function parseIntensity(value) {
    ...
    if (Object.hasOwn(ORDINALS, normalized))
 ...
 */
export function project(mutation) {
    switch (mutation.kind) {
        case "retire_skill":
            return {
                deltas: [RETIRE_SKILL_DELTA],
                explanation: `retiring skill "${mutation.target}" removes a known procedure, modelled as reduced ` +
...
function projectGenomeAmendment(mutation) {
    ...
    if (field === "preferences") {
        ...
        return {
            ...
            explanation: `preference "${suffix}" ${before === null ? "set to " : `moved ${before} → `}` +
                `${after}, projected onto ${traits.join(", ")} (${amount >= 0 ? "+" : ""}${amount.toFixed(2)})`,
        };
    }
    if (field === "policies") {
        const policy = (suffix === "remove" ? mutation.current_value : mutation.proposed_value)?.toLowerCase();
        if (!policy)
            return null;
...
export function explainUnprojectable(mutation) {
    switch (mutation.kind) {
        case "reconcile_belief":
...
export function applyDeltas(traits, deltas) {
    const next = { ...traits };
