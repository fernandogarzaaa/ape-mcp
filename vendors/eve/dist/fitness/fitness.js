...
import { definePersona, getPersona } from "../personas/index.js";
...
import { DEFAULT_SCENARIO_IDS, resolveScenarios } from "./scenarios.js";
/**
 ...
 */
export const DEFAULT_PANEL = ["first-time-user", "impatient-user", "power-user"];
 ...
 * what was measured.
 */
export async function validateMutation(request, options = {}) {
    ...
    const candidatePersonas = basePersonas.map((persona) => definePersona({
    ...
    for (const scenario of scenarios) {
        baselineScores.push(await scoreScenario(scenario, basePersonas, request.seed, trials, maxSteps));
        candidateScores.push(await scoreScenario(scenario, candidatePersonas, request.seed, trials, maxSteps));
    }
    ...
    const worstRegression = Math.min(0, ...candidateScores.map((score, i) => toBasisPoints(score.composite) -
        toBasisPoints(baselineScores[i].composite)));
    ...
    return build({
        ...
        evidence: [
            ...
            ...candidateScores.map((score, i) => `${score.scenarioId}: ${baselineScores[i].composite.toFixed(3)} → ${score.composite.toFixed(3)}`),
        ],
    });
}
/**
 ...
 */
async function scoreScenario(scenario, personas, seed, trials, maxSteps) {
    const successes = [];
    const overalls = [];
    const frustrations = [];
    const trusts = [];
    ...
    // sign.
    const composite = clamp01(taskSuccess * 0.4 +
        (overall / 100) * 0.3 +
...
function recommend(input) {
    ...
    if (deltaBp < thresholds.approveDeltaBp) {
        return {
            ...
            reason: `candidate moved the composite by ${deltaBp}bp, inside the noise band ` +
    ...
    if (worstRegressionBp < -thresholds.maxScenarioRegressionBp) {
        return {
            recommendation: "needs_review",
            reason: `candidate improved the composite by ${deltaBp}bp but regressed one scenario by ` +
    ...
    if (mutation.risk_bp > input.thresholds.maxAutoApproveRiskBp) {
        return {
            recommendation: "needs_review",
            reason: `candidate improved the composite by ${deltaBp}bp, but the mutation carries ` +
...
function unmeasurable(request, scenarioIds, trials, reason) {
