...
const registry = new Map(Object.keys(BENCHMARK_APPS).map((tier) => [
    tier,
    {
        id: tier,
 ...
 */
export const DEFAULT_SCENARIO_IDS = Object.keys(BENCHMARK_APPS);
/**
 ...
 * had run.
 */
export function resolveScenarios(ids) {
    return ids.map((id) => {
        const scenario = registry.get(id);
        if (!scenario) {
            throw new Error(`unknown scenario "${id}"; registered scenarios are ${[...registry.keys()].join(", ")}`);
        }
        return scenario;
    });
}
