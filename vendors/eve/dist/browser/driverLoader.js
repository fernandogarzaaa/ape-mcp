...
export class DriverMissingError extends Error {
    driver;
    installCommand;
    constructor(driver, installCommand) {
        ...
        const nextStep = driver === "playwright"
        ...
        super(`EVE's ${driver} adapter needs the "${driver}" package, which is not installed.\n` +
...
export class DriverLoadError extends Error {
    ...
    constructor(driver, cause) {
        ...
        super(`EVE's ${driver} adapter found the "${driver}" package but could not load it: ${detail}\n` +
 ...
 */
function isModuleNotFound(error, spec) {
 ...
 */
export async function importDriver(spec, installCommand) {
    try {
        return await import(spec);
    }
    catch (error) {
        if (isModuleNotFound(error, spec))
            throw new DriverMissingError(spec, installCommand);
 ...
 */
export function isMissingBrowserBinary(error) {
