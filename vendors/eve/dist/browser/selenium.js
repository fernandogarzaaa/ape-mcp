...
export class SeleniumAdapter {
    ...
    async open(url, viewport) {
        ...
        const key = (name, fallback) => webdriver.Key[name] ?? fallback;
        this.keyMap = {
            Enter: key("ENTER", ""),
            Tab: key("TAB", ""),
            Escape: key("ESCAPE", ""),
    ...
    requireDriver() {
        if (!this.driver)
            throw new Error("SeleniumAdapter: call open() first");
        return this.driver;
    }
    /**
...
async function importSelenium() {
    return (await importDriver("selenium-webdriver", "npm install selenium-webdriver"));
}
async function importChromeOptions() {
    try {
