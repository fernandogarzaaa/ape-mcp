...
export function isOptionalTransport(surface) {
 ...
 */
async function probeBrowser(surface, spec, installCommand, binaryCommand, provides, launch) {
    let mod;
    try {
...
export async function diagnoseSurfaces() {
    const browsers = await Promise.all([
        probeBrowser("playwright", "playwright", "npm install playwright", "npx playwright install chromium", "Web apps in a real browser (bundled — EVE's reference visual surface).", async (mod) => mod.chromium.launch({ headless: true })),
        probeBrowser("puppeteer", "puppeteer", "npm install puppeteer", "npx puppeteer browsers install chrome", "Web apps via Puppeteer. Same capability as Playwright, different transport.", async (mod) => {
            const m = mod;
            const pptr = (m.default ?? m);
    ...
    try {
        ...
        selenium = {
            ...
            detail: "The package is installed. EVE cannot confirm the surface works without starting a session: Selenium also needs a browser and a matching driver on PATH.",
        };
    }
    catch (error) {
        selenium = {
...
export function renderDoctor(reports) {
    ...
    // already going wrong.
    if (reports.length === 0)
        return "EVE surface check\n\n  No surfaces to check.\n";
    const mark = (s) => s === "ready" ? "✓" : s === "unverified" ? "?" : s === "needs-setup" ? "!" : "✗";
    ...
    for (const r of reports) {
        ...
        if (r.detail)
            lines.push(`${indent}${r.detail}`);
        if (r.remedy)
            lines.push(`${indent}Fix: ${r.remedy}`);
    }
    ...
    if (unusedTransports.length > 0) {
        lines.push(`  ${unusedTransports.length} alternative transport(s) unavailable. Nothing is lost:` +
