...
<main>
  ...
  <div class="score-grid">
    ${result.scores
        ...
        .map((s) => `<div class="score-card"><div>${escapeHtml(labelOf(s.dimension))}</div><b>${s.value}</b><div class="bar"><i style="width:${s.value}%;background:${scoreColor(s.value)}"></i></div><div class="ev">${escapeHtml(s.evidence[0] ?? "")}</div></div>`)
        .join("\n    ")}
  </div>
  ...
  ${result.workflows.length === 0
        ...
        : `<table><tr><th>Workflow</th><th>Screens</th><th>Completed</th><th>Errors</th></tr>${result.workflows
            .map((w) => `<tr><td>${escapeHtml(w.kind)}</td><td>${w.screens.length}</td><td>${w.completed ? "✅" : "❌"}</td><td>${w.errorCount}</td></tr>`)
            .join("")}</table>`}
  ...
  <div class="journal">
    ${result.iterations
        .map((it) => `<p><b>#${it.step}</b> <code>${escapeHtml(it.actionDescription)}</code><br><span class="r">${escapeHtml(it.rationale)}</span></p>`)
        .join("\n    ")}
  </div>
...
function emotionSvg(timeline) {
    const width = 940;
    const height = 220;
    const pad = 30;
    if (timeline.length < 2)
        return "<p><i>Not enough data for a timeline.</i></p>";
    const series = [
    ...
    const x = (i) => pad + (i / (timeline.length - 1)) * (width - pad * 2);
    ...
    const gridlines = [0, 0.25, 0.5, 0.75, 1]
        .map((v) => `<line x1="${pad}" y1="${y(v)}" x2="${width - pad}" y2="${y(v)}" stroke="#eef0f4"/><text x="4" y="${y(v) + 4}" font-size="10" fill="#98a2b3">${Math.round(v * 100)}</text>`)
        .join("");
...
function heatmapSvg(report) {
    const { result } = report;
    const clicks = result.iterations
        .filter((it) => it.clickPoint !== null)
        .map((it) => it.clickPoint);
    if (clicks.length === 0)
        return "<p><i>No pointer interactions were recorded.</i></p>";
    const vw = 1280;
    const vh = 800;
    const scale = 940 / vw;
    const circles = clicks
        .map((p, i) => `<circle cx="${(p.x * scale).toFixed(1)}" cy="${(p.y * scale).toFixed(1)}" r="14" fill="#d92d20" opacity="0.22"/><circle cx="${(p.x * scale).toFixed(1)}" cy="${(p.y * scale).toFixed(1)}" r="3" fill="#d92d20" opacity="0.8"><title>click #${i + 1}</title></circle>`)
        .join("");
    const background = result.screenshots.length > 0
...
function screenshotsSection(report) {
    ...
    const figures = shots
        .slice(0, maxShots)
        .map((buffer, i) => `<figure><img src="data:image/png;base64,${buffer.toString("base64")}" alt="Screenshot ${i}" loading="lazy"><figcaption>Screenshot ${i}</figcaption></figure>`)
        .join("\n    ");
