 ...
 * document.
 */
export function renderPanelMarkdown(panel) {
    ...
    push();
    push(`- Mean experience score: **${executive.meanOverallScore}/100** (range ${executive.scoreRange.min}–${executive.scoreRange.max})`);
    ...
    for (const p of executive.topPriorities)
        push(`1. ${p}`);
    push();
    push("## Consensus Issues (multiple personas agreed)");
    push();
    if (executive.consensusIssues.length === 0)
        push("_No consensus issues._");
    else {
    ...
    if (executive.disagreements.length > 0) {
        ...
        push();
        for (const d of executive.disagreements)
            push(`- **${d.topic}** — ${d.detail}`);
        push();
    }
...
//# sourceMappingURL=panelReport.js.map