...
function executiveSummary(result, overall) {
    ...
    const emotionalNote = finalEmotion
        ...
        : "";
    return (`A simulated "${result.personaName}" spent ${(result.usage.durationMs / 1000 / 60).toFixed(1)} simulated minutes ` +
...
function quickWins(findings) {
    ...
    for (const f of findings) {
        if (f.severity === "critical")
            continue; // criticals are rarely "quick"
    ...
    if (wins.length === 0)
        wins.push("No obvious quick wins — remaining issues need deeper work.");
    return wins;
}
function longTermImprovements(result) {
    const items = [];
    const scoreOf = (d) => result.scores.find((s) => s.dimension === d)?.value ?? 100;
    if (scoreOf("learnability") < 60) {
        items.push("Invest in learnability: the operator's mental model never converged — align labels, layouts and outcomes with common conventions so behavior becomes predictable.");
    }
    if (scoreOf("navigation") < 60 || scoreOf("informationArchitecture") < 60) {
        items.push("Rework the information architecture: the operator spent significant time hunting and backtracking. Card-sort the navigation and flatten deep or ambiguous paths.");
    }
    if (scoreOf("errorRecovery") < 60) {
        items.push("Design an error-recovery strategy: errors currently strand users. Every error state needs a plain-language explanation and one obvious way forward.");
    }
    if (scoreOf("responsiveness") < 60) {
        items.push("Set a performance budget: perceived latency repeatedly broke the operator's flow. Target <1s for interactions with immediate (<100ms) feedback.");
    }
    if (scoreOf("workflowQuality") < 60) {
        items.push("Audit core workflows end-to-end: discovered workflows could not be completed. Instrument funnels and remove dead ends.");
    }
    if (scoreOf("trust") < 55) {
        items.push("Rebuild trust signals: broken promises (errors, dead controls, surprises) eroded the operator's trust. Consistency and honest feedback are the fix, not visual polish.");
    }
    if (items.length === 0) {
