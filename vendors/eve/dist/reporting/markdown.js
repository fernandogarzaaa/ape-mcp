...
export function renderMarkdown(report) {
    ...
    push();
    const bySeverity = (sev) => result.findings.filter((f) => f.severity === sev);
    ...
    if (result.workflows.length === 0)
        push("No recognizable workflows were discovered.");
    else {
    ...
    for (const it of violations.slice(0, 20)) {
        push(`- **Step ${it.step}** — ${it.actionDescription}: expected "${it.prediction.description}" but got ${it.outcome.errorPerceived ? "an error" : it.outcome.screenChanged ? "an unrelated screen" : "no response"} (surprise ${pct(it.outcome.surprise)}).`);
    }
    push();
    ...
    for (const s of samples) {
        push(`| ${s.step} | ${pct(s.values.confidence)} | ${pct(s.values.frustration)} | ${pct(s.values.confusion)} | ${pct(s.values.trust)} | ${pct(s.values.fatigue)} |`);
    }
    push();
    push("## Session Journal");
    push();
    for (const it of result.iterations) {
        const emo = it.emotion;
        push(`- **#${it.step}** \`${it.actionDescription}\` — _${it.rationale}_ (frustration ${pct(emo.frustration ?? 0)})`);
    }
    push();
    ...
    if (result.cognitiveLoad) {
        ...
        if (worst) {
            ...
            push(`Heaviest screen (step ${worst.step}): working memory ${pct(b.workingMemoryLoad)}, decisions ${pct(b.decisionLoad)}, reading ${pct(b.informationLoad)}, clutter ${pct(b.visualClutter)}, task-switch ${pct(b.taskSwitchLoad)}.`);
        }
        push();
    }
    ...
    if (result.attention) {
        push("## Attention");
        push();
        push(`The operator made attention fixations across ${result.attention.fixations.length} glance(s). ` +
    ...
    push();
    for (const item of report.longTermImprovements)
        push(`- ${item}`);
    push();
    push("---");
