...
export function discoverJourney(goal, iterations, graph, outcome) {
    ...
    for (const it of iterations) {
        ...
        const o = it.outcome;
        if (o?.errorPerceived)
            friction.push("a visible error appeared");
        if (o?.prediction.expectsChange && !o.screenChanged)
            friction.push("the action produced no visible response");
        if (o && o.surprise > 0.6)
            friction.push("the result was surprising");
        if (it.action.kind === "back")
            friction.push("the operator backtracked");
        if (o && o.perceivedLatencyMs > 3000)
            friction.push(`a ${(o.perceivedLatencyMs / 1000).toFixed(1)}s wait`);
        const forwardProgress = it.url !== lastUrl &&
            it.action.kind !== "back" &&
            it.action.kind !== "read" &&
            it.action.kind !== "wait";
        if (!forwardProgress && (it.action.kind === "back" || friction.length > 0))
            wastedSteps += 1;
        const frustration = typeof it.emotion.frustration === "number"
            ? it.emotion.frustration
            : 0;
        steps.push({
            step: it.step,
            url: it.url,
    ...
    for (const s of steps) {
        if (path[path.length - 1] !== s.title)
            path.push(s.title);
    }
    const reachedTerminal = outcome.goalAchieved ||
    ...
    const durationMs = steps.length > 0
 ...
 */
export function inferJourneyIntent(journey) {
    const kinds = [
        ...new Set(journey.steps.map((s) => s.workflowKind).filter((k) => k !== "unknown")),
    ];
    if (kinds.length === 0)
        return "The operator explored without a recognizable journey.";
