...
export function computeScores(input) {
    ...
    const meanEmotion = (key) => {
        if (input.emotionTimeline.length === 0)
            return 0.5;
        let sum = 0;
        for (const s of input.emotionTimeline)
            sum += s.values[key];
        return sum / input.emotionTimeline.length;
    };
    const peakEmotion = (key) => {
        let max = 0;
        for (const s of input.emotionTimeline)
            max = Math.max(max, s.values[key]);
        return max;
    };
    const bySeverity = (severity) => input.findings.filter((f) => f.severity === severity);
    const byCategory = (category) => input.findings.filter((f) => f.category === category);
    const findingPenalty = (findings, perCritical = 25, perMajor = 12, perMinor = 4) => {
        let penalty = 0;
        for (const f of findings) {
            penalty +=
                f.severity === "critical"
    ...
    /* ---- usability -------------------------------------------------- */
    {
        ...
        if (evidence.length === 0)
            evidence.push("Interactions consistently matched expectations.");
    ...
    /* ---- learnability ----------------------------------------------- */
    {
        ...
        const rate = (xs) => xs.length ? xs.filter((o) => o.surprise > 0.5).length / xs.length : 0;
        const improvement = rate(early) - rate(late);
        const value = 70 + improvement * 60 - peakEmotion("confusion") * 25;
        if (outcomes.length >= 6) {
            evidence.push(`Expectation-violation rate went from ${(rate(early) * 100).toFixed(0)}% (first half) to ${(rate(late) * 100).toFixed(0)}% (second half).`);
        }
    ...
    {
        const evidence = [];
        const productiveSteps = input.iterations.filter((it) => it.action.kind !== "wait" && it.action.kind !== "read").length;
    ...
    /* ---- visual design ----------------------------------------------- */
    {
        ...
        if (evidence.length === 0)
            evidence.push("No visual defects (contrast, clipping, overflow, misalignment) were perceived.");
    ...
    /* ---- navigation --------------------------------------------------- */
    {
        ...
        const value = 88 - input.revisitRatio * 40 - backSteps * 4 - findingPenalty(byCategory("navigation"));
        if (backSteps > 0)
            evidence.push(`The operator backtracked ${backSteps} time(s).`);
    ...
    /* ---- workflow quality --------------------------------------------- */
    {
        ...
        if (discovered.length > 0) {
            ...
            evidence.push(`Discovered ${discovered.length} workflow(s): ${discovered.map((w) => w.kind).join(", ")}.`);
            if (completed.length)
    ...
    /* ---- information architecture -------------------------------------- */
    {
        ...
        const searchy = input.iterations.filter((it) => it.action.kind === "scroll" || it.action.kind === "back").length;
        ...
        evidence.push(`${(ratio * 100).toFixed(0)}% of steps were spent hunting (scrolling/backtracking) rather than acting.`);
        setScore("informationArchitecture", { value: clamp(value, 0, 100), evidence });
    }
    /* ---- onboarding ----------------------------------------------------- */
    {
        const evidence = [];
        const firstQuarter = input.emotionTimeline.slice(0, Math.max(1, Math.floor(input.emotionTimeline.length / 4)));
        let earlyConfusion = 0;
        for (const s of firstQuarter)
            earlyConfusion = Math.max(earlyConfusion, s.values.confusion);
        ...
        if (sawOnboarding)
            evidence.push("An onboarding/welcome flow was present and encountered.");
    ...
    if (input.modality) {
        for (const entry of dimensionRegistry.listFor(input.modality)) {
            if (entry.builtin || results.has(entry.id))
                continue;
