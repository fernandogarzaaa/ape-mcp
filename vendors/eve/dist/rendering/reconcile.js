...
import { backgroundLuminance, cellFor, inkDensity, inkGrid, meanLuminance, } from "./ink.js";
 ...
 */
function perceive(screenshot, viewport) {
    if (!screenshot || viewport.width <= 0 || viewport.height <= 0)
        return null;
    let img;
    try {
...
export function observe(screenshot, viewport) {
 ...
 */
function fullyVisible(box, viewport) {
    return (box.x >= 0 &&
        box.y >= 0 &&
 ...
 */
export function abbreviate(text, max = 60) {
 ...
 */
export function reconcile(percept, observation) {
...
function reconcileWith(percept, observation, grid) {
    ...
    for (const region of observation.regions) {
        if (region.kind !== "text" && region.kind !== "graphic")
            continue;
        const area = region.box.width * region.box.height;
        if (area < MIN_NOTICEABLE_AREA)
            continue;
        if (!fullyVisible(region.box, viewport))
            continue;
        ...
        for (const box of domBoxes) {
            ...
            if (covered >= area * ACCOUNTED_COVERAGE)
                break;
        }
        if (covered >= area * ACCOUNTED_COVERAGE)
            continue;
...
export function inspect(percept) {
