...
export function cellIndex(grid, cx, cy) {
 ...
 */
export function inkGrid(img, cell = CELL) {
        ...
        for (let cx = 0; cx < width; cx++) {
            ...
            for (let y = y0; y < y1; y++) {
                for (let x = x0; x < x1; x++) {
                    ...
                    const l = relativeLuminance(img.data[o], img.data[o + 1], img.data[o + 2]);
                    sum += l;
                    sumSq += l * l;
                    n++;
                }
 ...
 */
export function backgroundLuminance(grid) {
    const BUCKETS = 32;
    ...
    if (best < 0)
        return 1; // Every cell carries detail; assume a light page.
    return histogram[best] / counts[best];
}
/**
...
function cellRange(grid, box) {
    ...
    if (inner.cx1 > inner.cx0 && inner.cy1 > inner.cy0)
        return inner;
    const outer = {
 ...
 */
export function inkDensity(grid, box) {
    ...
    for (let cy = range.cy0; cy < range.cy1; cy++) {
        for (let cx = range.cx0; cx < range.cx1; cx++) {
            total++;
            if (grid.ink[cy * grid.width + cx] === 1)
                inked++;
        }
    }
...
export function meanLuminance(grid, box, fallback) {
    const range = cellRange(grid, box);
    if (!range)
        return fallback;
    let sum = 0;
    ...
    for (let cy = range.cy0; cy < range.cy1; cy++) {
        for (let cx = range.cx0; cx < range.cx1; cx++) {
            sum += grid.luminance[cy * grid.width + cx];
            n++;
        }
    }
