 ...
 */
import { inkDensity, meanLuminance } from "./ink.js";
/**
...
function findBlobs(grid) {
    ...
    for (let startY = 0; startY < grid.height; startY++) {
        for (let startX = 0; startX < grid.width; startX++) {
            ...
            if (seen[start] === 1 || grid.ink[start] === 0)
                continue;
            const blob = {
                minX: startX,
                minY: startY,
            ...
            while (stack.length > 0) {
                ...
                for (let dx = -H_BRIDGE; dx <= H_BRIDGE; dx++) {
                    const nx = x + dx;
                    if (nx < 0 || nx >= grid.width || dx === 0)
                        continue;
                    const n = y * grid.width + nx;
                    if (seen[n] === 1 || grid.ink[n] === 0)
                        continue;
                    seen[n] = 1;
                    stack.push(n);
                }
...
function absorb(a, b) {
    ...
    a.cells += b.cells;
    for (const [row, count] of b.rowInk)
        a.rowInk.set(row, (a.rowInk.get(row) ?? 0) + count);
}
/**
 ...
 */
function groupIntoBlocks(blobs) {
    const ordered = [...blobs].sort((a, b) => a.minY !== b.minY ? a.minY - b.minY : a.minX - b.minX);
    const blocks = [];
    for (const blob of ordered) {
        let merged = false;
        for (const block of blocks) {
            const gap = blob.minY - block.maxY - 1;
            if (gap > V_JOIN || gap < -Math.max(1, block.maxY - block.minY + 1))
                continue;
            ...
            if (overlap <= 0 || overlap / narrower < COLUMN_OVERLAP)
                continue;
            absorb(block, blob);
            merged = true;
            break;
...
function classify(density, structure, contrastWithPage) {
    if (density < 0.08)
        return contrastWithPage > 0.08 ? "solid" : "blank";
    if (structure >= 0.5)
        return "text";
 ...
 */
export function findRegions(grid, scale, pageLuminance) {
    const regions = [];
