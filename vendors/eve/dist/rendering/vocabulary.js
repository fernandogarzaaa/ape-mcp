...
export function registerRenderingVocabulary() {
    if (registered)
        return;
    registered = true;
    if (!dimensionRegistry.has(RENDERING_DIMENSION)) {
        registerDimension({
            ...
            description: "Whether the page renders what it claims: no controls that exist only in the markup, no text that never reached the screen, and nothing on screen that only a pair of eyes can reach.",
