...
export function renderStudyJson(study) {
 ...
 */
export function renderOperatorCsv(study) {
    const header = CSV_COLUMNS.join(",");
    const rows = study.operators.map((op) => CSV_COLUMNS.map((col) => {
...
function dist(label, d) {
    return (`- **${label}:** mean ${d.mean}, sd ${d.stdDev}, ` +
        `median ${d.median} (p25 ${d.p25} – p75 ${d.p75}), range ${d.min}–${d.max}`);
}
/** A complete, human-readable study report. */
export function renderStudyMarkdown(study) {
    ...
    for (const seg of study.segments) {
        lines.push(`- **${seg.name}** — ${seg.size} operators (${pct(seg.share)}), ` +
    ...
    for (const entry of study.navigationHeatmap.slice(0, 12)) {
        lines.push(`- \`${entry.screen}\` — ${entry.visits} visits across ${entry.operators} operators (reach ${pct(entry.reach)})${entry.dropoffs ? `, ${entry.dropoffs} drop-off${entry.dropoffs > 1 ? "s" : ""} here` : ""}`);
    }
    if (study.topFindings.length) {
        lines.push("", "## Most common findings (population-wide)");
        for (const f of study.topFindings) {
            lines.push(`- **[${f.severity}] ${f.title}** — hit by ${f.operatorsAffected} operators (${pct(f.prevalence)})${f.recommendation ? `. Fix: ${f.recommendation}` : ""}`);
        }
    }
    lines.push("");
    return lines.join("\n");
...
export function renderStudy(study, format) {
    switch (format) {
        case "json":
