...
function fitPowerLaw(values) {
    const points = values
        .map((v, i) => ({ x: Math.log(i + 1), y: Math.log(Math.max(1e-6, v)) }))
        .filter((p) => Number.isFinite(p.y));
    const n = points.length;
    if (n < 2)
        return { alpha: 0, r2: 0, a: values[0] ?? 0 };
...
export function computeLearningMetrics(memory) {
    ...
    const timeReductionRatio = first && last && first.durationMs > 0 ? last.durationMs / first.durationMs : 1;
    ...
    const retention = allStrengths.length > 0 ? allStrengths.reduce((a, b) => a + b, 0) / allStrengths.length : 0;
    return {
 ...
 */
export function forgettingCurve(retentionTrait, gaps = 10) {
...
export function renderLearningCurveSvg(metrics, options = {}) {
