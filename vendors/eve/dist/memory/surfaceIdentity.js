 ...
 */
export function canonicalSurfaceId(c) {
...
export function canonicalFromPercept(percept, taskId = null) {
    return {
        kind: "eve-stable",
        taskId,
        url: percept.url,
 ...
 */
export function canonicalMatchBasis(a, b) {
    const tasksAgree = !a.taskId || !b.taskId || normalizeTaskId(a.taskId) === normalizeTaskId(b.taskId);
    if (!tasksAgree)
        return null;
    ...
    if (a.url && b.url && a.url === b.url)
        return taskPrefix ? "task+url" : "url";
    return null;
}
