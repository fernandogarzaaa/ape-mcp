...
import { createAdapter, diagnoseSurfaces, isOptionalTransport, renderDoctor, } from "../browser/index.js";
...
import { loadConfigFile, resolveConfig } from "../config/config.js";
import { converse, DEMO_SUPPORT_BOT, HttpBackend, renderConversationMarkdown, ScriptedBackend, } from "../conversation/index.js";
import { EveSession } from "../engine/session.js";
import { artifactWordCount, DOC_SCHEME, docTargetOf, readArtifact, renderComprehensionMarkdown, } from "../humanity/index.js";
...
import { applyProfession, definePersona, getCulture, getPersona, getProfession, listCultures, listPersonas, listProfessions, registerPersona, } from "../personas/index.js";
import { AccessibilityPlugin, LlmCriticPlugin, LocalizationPlugin, PerformancePlugin, } from "../plugins/index.js";
...
export async function main(argv = process.argv.slice(2)) {
    ...
    try {
        ...
        if (values.persona)
            config.persona = values.persona;
        if (values.goal)
            config.goal = values.goal;
        if (values.browser)
            config = resolveConfig({ ...configToRaw(config), browser: values.browser });
        if (values.device)
            config.device = values.device;
        if (values.steps)
            config.maxSteps = parsePositiveInt(values.steps, "--steps");
        if (values.minutes)
            config.maxDurationMinutes = parsePositiveInt(values.minutes, "--minutes");
        if (values.seed)
            config.seed = /^\d+$/.test(values.seed) ? Number(values.seed) : values.seed;
        if (values.strategy) {
            config = resolveConfig({ ...configToRaw(config), explorationStrategy: values.strategy });
        }
        if (values.headed)
            config.headless = false;
        if (values["no-screenshots"])
            config.screenshots = false;
        if (values.llm)
            config.llmCognition = true;
        if (values["llm-critic"])
            config.plugins.llmCritic = true;
        if (values.out)
            config.outputDir = values.out;
        if (values.quiet)
            config.verbosity = "quiet";
        if (values.cognitive)
            config.cognitive = true;
        if (values.utility)
            config.utilityDecisions = true;
        if (values.culture)
            config.culture = getCulture(values.culture).locale;
        if (values.profession)
            config.profession = getProfession(values.profession).name;
        if (values.remember)
            config.longTermMemoryPath = values.remember;
    }
    catch (err) {
    ...
    for (const spec of config.customPersonas ?? [])
        registerPersona(definePersona(spec));
    let persona = typeof config.persona === "string" ? getPersona(config.persona) : config.persona;
    if (config.profession)
        persona = applyProfession(persona, getProfession(config.profession));
    const plugins = [];
    if (config.plugins.accessibility)
        plugins.push(new AccessibilityPlugin());
    if (config.plugins.performance)
        plugins.push(new PerformancePlugin());
    if (config.culture)
        plugins.push(new LocalizationPlugin());
    if (config.plugins.llmCritic) {
        plugins.push(new LlmCriticPlugin(typeof config.plugins.llmCritic === "object" ? config.plugins.llmCritic : {}));
    }
...
async function runStudyCommand(rest) {
    const { values, positionals } = parseArgs({
        args: [...rest],
        allowPositionals: true,
        options: {
    ...
    try {
        ...
        if (format === "json") {
            process.stdout.write(`${JSON.stringify({ study, ...(report ? { moderated: report } : {}), ...(intel ? { product: intel } : {}) }, null, 2)}\n`);
        }
        else {
...
/** Serialize the parts of a config that resolveConfig re-validates. */ function configToRaw(config) {
    return {
        url: config.url,
...
async function runMcpEvalCommand(rest) {
    const { values, positionals } = parseArgs({
        args: [...rest],
        allowPositionals: true,
        options: {
 ...
 */
async function runReadCommand(rest) {
    let values;
    let positionals;
    try {
    ...
    try {
        const result = await readArtifact(target, {
            persona,
            ...(typeof values.genre === "string" ? { genre: values.genre } : {}),
            ...(typeof values.format === "string" ? { format: values.format } : {}),
        ...
        const written = await writeReports(result, typeof values.out === "string" ? values.out : ".eve-output");
        const critical = result.findings.filter((f) => f.severity === "critical").length;
        const major = result.findings.filter((f) => f.severity === "major").length;
        const artifact = result.artifact;
        const noun = artifact.sections[0]?.noun ?? "section";
        process.stdout.write(`\n${"─".repeat(64)}\n`);
        process.stdout.write(`Artifact                 : ${artifact.genre}, ${artifact.sections.length} ${noun}(s), ${artifactWordCount(artifact)} words (${artifact.format})\n`);
        process.stdout.write(`Understood               : ${result.comprehension.comprehensionScore}/100\n`);
        process.stdout.write(`Reading ease             : Flesch ${result.comprehension.readability.fleschReadingEase} (grade ${result.comprehension.readability.gradeLevel})\n`);
        process.stdout.write(`Reading time             : ${(result.comprehension.readingTimeMs / 60000).toFixed(1)} min at this reader's pace\n`);
        process.stdout.write(`Findings                 : ${critical} critical, ${major} major, ${result.findings.length - critical - major} other\n`);
        process.stdout.write(`Outcome                  : ${result.endReason}${result.abandonReason ? ` — ${result.abandonReason}` : ""}\n`);
        process.stdout.write(`Reports                  : ${written.html}\n`);
        if (typeof values.report === "string") {
...
async function runChatCommand(rest) {
    ...
    const kinds = ["support", "assistant", "copilot", "scripted"];
    if (typeof values.kind === "string" && !kinds.includes(values.kind)) {
    ...
    if (!isMock && !/^https?:\/\//i.test(target)) {
        process.stderr.write(`"eve chat" needs an http(s) chat endpoint, or "mock:" for the offline demo; got "${target}".\n`);
        return 2;
    }
    ...
    try {
        const result = await converse(backend, {
            ...
            ...(typeof values.kind === "string" ? { kind: values.kind } : {}),
        ...
        process.stdout.write(`\n${"─".repeat(64)}\n`);
        process.stdout.write(`Conversation             : ${c.kind}, ${c.turnCount} turn(s), asked ${c.operatorTurns} time(s)\n`);
        process.stdout.write(`Understood the person    : ${c.understanding}/100 (${c.silentMisses} silent miss(es), ${c.admittedMisses} admitted)\n`);
        process.stdout.write(`Showed it understood     : ${c.grounding}/100\n`);
        process.stdout.write(`Recovered when it failed : ${c.recovery}/100 (${c.everOfferedHandoff ? "offered a person" : "never offered a person"})\n`);
        process.stdout.write(`Had to rephrase          : ${c.repairAttempts}×\n`);
        if (c.meanLatencyMs !== null) {
            process.stdout.write(`Reply time               : mean ${(c.meanLatencyMs / 1000).toFixed(1)}s, slowest ${((c.maxLatencyMs ?? 0) / 1000).toFixed(1)}s\n`);
        }
        process.stdout.write(`Findings                 : ${critical} critical, ${major} major, ${result.findings.length - critical - major} other\n`);
        process.stdout.write(`Outcome                  : ${result.endReason}${result.abandonReason ? ` — ${result.abandonReason}` : ""}\n`);
        process.stdout.write(`Reports                  : ${written.html}\n`);
