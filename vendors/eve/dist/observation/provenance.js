 ...
 */
export function visualOnlyText(percept) {
    const parts = [];
    if (percept.title)
        parts.push(percept.title);
    for (const el of percept.elements) {
        if (isHumanVisibleText(el) && el.text)
            parts.push(el.text);
    }
...
export function accessibilitySourcedText(percept) {
...
export function provenanceOf(kind) {
    switch (kind) {
        case "observed":
            return "observed";
        case "derived":
