...
export function renderMultimodalMarkdown(report) {
    const lines = [
    ...
    for (const t of report.toasts.slice(0, 8))
        lines.push(`  - "${t.label}" on \`${t.screen}\``);
    ...
    if (report.unlabeled.length === 0) {
        ...
        for (const u of report.unlabeled)
            grouped.set(u.kind, (grouped.get(u.kind) ?? 0) + 1);
