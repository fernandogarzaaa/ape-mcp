...
export class HeuristicMultimodalPerceptor {
    name = "heuristic";
    perceive(percept, previous) {
        const cues = [];
        for (const el of percept.elements) {
            ...
            if (el.role === "image") {
                ...
                if (text)
                    cues.push({ kind: "text-in-image", label: text, accessible: true });
...
export const DEFAULT_MULTIMODAL_PERCEPTOR = new HeuristicMultimodalPerceptor();
//# sourceMappingURL=perceptor.js.map