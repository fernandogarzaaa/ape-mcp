import { contrastRatio, decodePng, frameDiffRatio, luminanceVariance, parseHexColor, relativeLuminance, sampleLuminances, } from "./pixels.js";
/* ------------------------------------------------------------------ */
/* Geometry checks (no pixels needed)                                 */
/* ------------------------------------------------------------------ */
export function checkGeometry(percept, accessibility) {
    const issues = [];
    ...
    const overflowing = meaningful.filter((el) => el.box.x + el.box.width > viewport.width + 8 && el.box.x < viewport.width);
...
function checkDeclaredContrast(percept, accessibility) {
    const issues = [];
    for (const el of percept.elements) {
        if (!el.text.trim() || el.text.trim().length < 3)
            continue;
 ...
 */
export function simulateColorVision([r, g, b], kind) {
    switch (kind) {
        case "protanopia":
            return [
...
/* ------------------------------------------------------------------ */
export function checkPixels(percept) {
    ...
    for (const el of percept.elements) {
        if (!el.text.trim() || el.text.trim().length < 4)
            continue;
        if (el.color && el.backgroundColor)
            continue; // declared-color path covers it
        if (el.box.width < 8 || el.box.height < 8)
            continue;
        if (el.box.y < 0 || el.box.y > percept.viewport.height)
            continue;
 ...
 */
export function checkRegression(previousShot, currentShot, sameTextContent) {
    try {
