...
export var BrowserBackend;
(function (BrowserBackend) {
    BrowserBackend["MOCK"] = "mock";
    BrowserBackend["PLAYWRIGHT"] = "playwright";
    BrowserBackend["PUPPETEER"] = "puppeteer";
    BrowserBackend["SELENIUM"] = "selenium";
    BrowserBackend["MOBILE"] = "mobile";
...
export const RunSessionSchema = z
    ...
    url: z
        .string()
        .min(1)
        .describe("Target to validate. A real URL (https://…) drives a real browser, " +
    ...
    persona: z
        ...
        .describe("Persona to simulate (e.g. first-time-user, impatient-user, " +
        "power-user, elderly-user, accessibility-user). Use eve_list_personas " +
        "to see the catalog."),
    goal: z
        .string()
        .optional()
        .describe("The task the operator is trying to accomplish (e.g. 'sign up for an " +
        "account'). Omit for open-ended exploration."),
    goal_success_signals: z
        .array(z.string())
        .default([])
        .describe("Visible text fragments that all must appear for the goal to count as " +
        "achieved (e.g. ['invitation sent'])."),
    profession: z
        .string()
        .optional()
        .describe("Optional professional overlay (doctor, accountant, lawyer, …) applied " +
        "to the persona. Use eve_list_professions for the catalog."),
    culture: z
        .string()
        .optional()
        .describe("Optional cultural profile / locale (en-US, de-DE, ja-JP, ar-SA, …). " +
        "Use eve_list_cultures for the catalog."),
    browser: z
        .enum(BrowserBackend)
        .optional()
        .describe("Browser backend. Defaults to `mock` for `mock:` URLs and `playwright` " +
        "for real URLs. Real backends must be installed as peer dependencies. " +
        "Use `mobile` to emulate a touch device (see `device`)."),
    device: z
        .string()
        .optional()
        .describe('Device to emulate when browser is "mobile" (default "iPhone 14"): ' +
    ...
    seed: z
        ...
        .optional()
        .describe("Reproducibility seed. The same (app, persona, seed) yields the same " +
        "session — always set this when comparing runs."),
    cognitive: z
        .boolean()
        .default(false)
        .describe("Enable the enhanced cognitive suite: selective attention, cognitive " +
        "load, trust, and the expectation engine."),
    utility: z
        .boolean()
        .default(false)
        .describe("Use utility-based decision-making (softmax over emotion-weighted " +
        "expected value) instead of the default heuristic policy."),
    remember_file: z
        .string()
        .optional()
        .describe("Path to a JSON memory file. When set, the operator carries learned app " +
        "knowledge across runs — run repeatedly against the same app to see it " +
        "get faster (power law of practice)."),
    output_dir: z
        .string()
        .default(".eve-output")
        .describe("Directory to write the full report.html / report.md / report.json. " +
    ...
    response_format: z
        ...
        .describe("Output format: 'markdown' for a human-readable summary or 'json' for " +
...
export const RunUsabilityStudySchema = z
    ...
    url: z
        .string()
        .min(1)
        .describe("Target to study. A real URL drives real browsers (one per operator), " +
    ...
    personas: z
        .array(z.string())
        .default([])
        .describe("Persona names to sample from (round-robin). Empty = the whole " +
    ...
    output_dir: z
        .string()
        .optional()
        .describe("If set, write the full research dataset (study.json, operators.csv, " + "study.md) here."),
    response_format: z
        .nativeEnum(ResponseFormat)
        .default(ResponseFormat.MARKDOWN)
...
export const ReadArtifactSchema = z
    ...
    target: z
        .string()
        .min(1)
        .describe("File path or http(s) URL. Standard input (`-`) is not available here — the MCP server uses stdio for the protocol itself."),
...
export const EvaluateConversationSchema = z
    ...
    goal: z
        ...
        .describe("What the person came for — becomes their opening line, so phrase it as they would say it."),
    goal_success_signals: z
        .array(z.string())
        .default([])
    ...
    reply_path: z
        .string()
        .optional()
        .describe("Dotted path to the reply text, e.g. choices.0.message.content. Common shapes are tried by default."),
    headers: z
    ...
    body_template: z
        .string()
        .optional()
        .describe('Request body template; {{message}} is substituted. Default {"message": "{{message}}"}.'),
...
export const GetReportSchema = z
    ...
    output_dir: z
        .string()
        .default(".eve-output")
        .describe("The directory a prior eve_run_session wrote to (same value as its " + "output_dir)."),
    format: z
        .enum(["markdown", "json"])
        .default("markdown")
        .describe("Which report file to read: 'markdown' (report.md, best for reading) or " +
