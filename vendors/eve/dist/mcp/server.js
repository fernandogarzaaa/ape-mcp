...
import { ApplicationMapSchema, BenchmarkSchema, CalibrateSchema, CompareBuildsSchema, EvaluateConversationSchema, EveBenchSchema, GetReportSchema, ListSchema, MultimodalScanSchema, ReadArtifactSchema, ResponseFormat, RunSessionSchema, RunUsabilityStudySchema, TwinSessionSchema, } from "./schemas.js";
import { compareBuilds, getReport, listCulturesTool, listPersonasTool, listProfessionsTool, runApplicationMap, runBenchmark, runCalibrate, runEvaluateConversation, runEveBenchTool, runMultimodalScan, runPredictUX, runProductReport, runReadArtifact, runSession, runTwinSessionTool, runUsabilityStudy, runUserStudy, ToolInputError, } from "./tools.js";
...
function respond(output, format) {
    const text = format === ResponseFormat.JSON ? JSON.stringify(output.structured, null, 2) : output.markdown;
    return { content: [{ type: "text", text }] };
}
/** Turn any thrown error into an actionable MCP tool error result. */
function fail(error) {
    const message = error instanceof ToolInputError
        ? error.message
...
export function createServer() {
    ...
    server.registerTool("eve_run_session", {
        title: "Run an EVE experience-validation session",
        description: "Simulate a realistic human (a persona with reading speed, memory, " +
    ...
    server.registerTool("eve_run_usability_study", {
        title: "Run a population usability study",
        description: "Simulate a whole population of varied operators (different personas, " +
    ...
    server.registerTool("eve_run_user_study", {
        title: "Run an AI-moderated user study",
        description: "Run a full autonomous user study: simulate a population, then convene a " +
    ...
    server.registerTool("eve_predict_ux", {
        title: "Predict UX with confidence intervals",
        description: "Run a population, then extrapolate to the wider user base: predicted " +
    ...
    server.registerTool("eve_product_report", {
        title: "Infer product intelligence",
        description: "Run a population, then infer product-level intelligence from how it " +
    ...
    server.registerTool("eve_multimodal_scan", {
        title: "Scan an app's multimodal perception",
        description: "Explore an app and analyze its perception beyond text: icons, charts, " +
    ...
    server.registerTool("eve_read_artifact", {
        title: "Read a digital output like a human",
        description: "Put a simulated reader in front of something software *produced* " +
    ...
    server.registerTool("eve_evaluate_conversation", {
        title: "Evaluate a conversational interface",
        description: "Put a simulated person in front of something that answers back — a " +
    ...
    server.registerTool("eve_calibrate", {
        title: "Calibrate EVE against a human study",
        description: "Import anonymized human usability traces from a JSON file, run a " +
    ...
    server.registerTool("eve_twin_session", {
        title: "Run a session as a persistent digital twin",
        description: "Run one session as a named, persistent digital twin — a user model that " +
    ...
    server.registerTool("eve_application_map", {
        title: "Autonomously map an application",
        description: "Given only a URL, explore the app with several curious operators and " +
    ...
    server.registerTool("eve_compare_builds", {
        title: "Compare experience across builds",
        description: "Run a usability study on each of several builds (ordered oldest to " +
    ...
    server.registerTool("eve_list_personas", {
        title: "List EVE personas",
        description: "List the built-in personas that eve_run_session can simulate " +
    ...
    }, async (input) => respond(listPersonasTool(), input.response_format));
    server.registerTool("eve_list_professions", {
        title: "List EVE professional overlays",
        description: "List the professional overlays (doctor, accountant, lawyer, designer, " +
        ...
        inputSchema: ListSchema.shape,
        annotations: READ_ONLY,
    }, async (input) => respond(listProfessionsTool(), input.response_format));
    server.registerTool("eve_list_cultures", {
        title: "List EVE cultural profiles",
        description: "List the cultural profiles / locales (en-US, de-DE, ja-JP, ar-SA, …) " +
            "that can be applied via eve_run_session's `culture` argument, affecting " +
            "reading direction, formats, and expectations.",
        inputSchema: ListSchema.shape,
        annotations: READ_ONLY,
    }, async (input) => respond(listCulturesTool(), input.response_format));
    server.registerTool("eve_bench", {
        title: "Run the EVE Bench scorecard",
        description: "Run the formal EVE Bench platform: a suite of known-quality reference " +
    ...
    server.registerTool("eve_benchmark", {
        ...
        description: "Run EVE against known-good and known-bad reference apps and check that " +
    ...
    server.registerTool("eve_get_report", {
        title: "Read a full EVE report",
        description: "Read back the full report a prior eve_run_session wrote to disk — the " +
        ...
        try {
            ...
            return { content: [{ type: "text", text: output.markdown }] };
        }
        catch (error) {
            return fail(error);
        }
