...
import { calibrate, importHumanStudy, renderCalibrationMarkdown, sanitizeHumanStudy, } from "../calibration/index.js";
...
import { applyProfession, getCulture, getPersona, getProfession, listCultures, listPersonas, listProfessions, } from "../personas/index.js";
...
function journalHighlights(result, limit = 3) {
...
function resolvePersona(input) {
    ...
    try {
        ...
        const names = listPersonas()
            .map((p) => p.name)
            .join(", ");
        throw new ToolInputError(`Unknown persona "${input.persona}". Available personas: ${names}. (Call eve_list_personas for descriptions.)`);
    }
    if (input.profession) {
        try {
            ...
            const names = listProfessions()
                .map((p) => p.name)
                .join(", ");
            throw new ToolInputError(`Unknown profession "${input.profession}". Available: ${names}. (Call eve_list_professions.)`);
        }
    }
    return persona;
}
function resolveCulture(input) {
    if (!input.culture)
        return undefined;
    try {
        ...
        const locales = listCultures()
            .map((c) => c.locale)
            .join(", ");
        throw new ToolInputError(`Unknown culture "${input.culture}". Available locales: ${locales}. (Call eve_list_cultures.)`);
    }
}
/**
...
export async function runSession(input) {
    ...
    const policy = input.utility ? new UtilityCognition() : new HeuristicCognition();
    ...
    const onLog = debug ? (line) => process.stderr.write(`[eve] ${line}\n`) : undefined;
    let adapter;
    try {
        adapter = createAdapter(browser, { headless: true, device: input.device });
    }
    catch (err) {
        throw new ToolInputError(`Could not start the "${browser}" browser backend: ` +
    ...
    if (structured.learning) {
        ...
        lines.push(`**Learning (this app):** session ${l.sessions}, learning rate ` +
            `${l.learningRate}, steps ${l.steps.join("→")}`);
    }
    if (structured.cognitiveLoad) {
        lines.push(`**Cognitive load:** mean ${structured.cognitiveLoad.mean}, peak ` +
    ...
    if (structured.journalHighlights.length) {
        ...
        for (const h of structured.journalHighlights)
            lines.push(`- ${h}`);
        lines.push("");
    }
    lines.push(`Full report: ${written.markdown} · ${written.html} · ${written.json}`, `Read the full markdown back with eve_get_report(output_dir="${input.output_dir}").`);
...
function toPopulationOptions(input) {
    return {
        url: input.url,
        size: input.size,
...
export async function runUsabilityStudy(input) {
    const study = await simulatePopulation(toPopulationOptions(input));
    let dataset = null;
    if (input.output_dir)
        dataset = await writeStudyDataset(study, input.output_dir);
    ...
    const markdown = truncate(renderStudyMarkdown(study) +
        (dataset
 ...
 */
export async function runUserStudy(input) {
    const study = await simulatePopulation(toPopulationOptions(input));
    ...
    const markdown = truncate(renderModeratedStudyMarkdown(report) +
 ...
 */
export async function runProductReport(input) {
    const study = await simulatePopulation(toPopulationOptions(input));
    ...
    const structured = { ...intel, file };
    const markdown = truncate(renderProductIntelligenceMarkdown(intel) + (file ? `\n\nWritten: ${file}` : ""));
    return { markdown, structured };
}
/**
 ...
 */
export async function compareBuilds(input) {
    const builds = [];
 ...
 */
export async function runPredictUX(input) {
    const study = await simulatePopulation(toPopulationOptions(input));
    ...
    const structured = { ...prediction, file };
 ...
 */
export async function runCalibrate(input) {
    let human;
    try {
        ...
        throw new ToolInputError(`Could not read the human study at "${input.human_file}": ${err instanceof Error ? err.message : String(err)}`);
    }
    const study = await simulatePopulation({
        url: input.url,
        size: input.size,
 ...
 */
export async function runTwinSessionTool(input) {
    const store = new FileTwinStore(input.twin_file);
    let twin = await store.load(input.twin_id);
    if (!twin) {
        if (!input.name || !input.base_persona) {
            throw new ToolInputError(`Twin "${input.twin_id}" does not exist yet — provide \`name\` and \`base_persona\` to create it.`);
        }
        try {
            twin = createTwin({
                id: input.twin_id,
    ...
    try {
        ...
        throw new ToolInputError(`Could not start the "${browser}" browser backend: ${err instanceof Error ? err.message : String(err)}`);
    }
 ...
 */
export async function runMultimodalScan(input) {
    const isMock = input.url.startsWith("mock:");
    const browser = input.browser ?? (isMock ? "mock" : "playwright");
    let adapter;
    try {
 ...
 */
export async function runReadArtifact(input) {
    ...
    if (input.target.trim() === "-") {
        throw new ToolInputError("This tool cannot read standard input: the MCP server uses stdio for the protocol itself. Pass a file path or an http(s) URL instead.");
    }
    let persona;
    try {
        persona = getPersona(input.persona);
    ...
    try {
        ...
        throw new ToolInputError(`Could not read "${input.target}": ${err instanceof Error ? err.message : String(err)}`);
    }
 ...
 */
export async function runEvaluateConversation(input) {
    let persona;
    try {
        persona = getPersona(input.persona);
        if (input.profession)
    ...
    if (!isMock && !/^https?:\/\//i.test(input.target)) {
        throw new ToolInputError(`"${input.target}" is not a chat endpoint. Pass an http(s) URL, or "mock:" for the demo bot.`);
    }
    ...
    const markdown = [
        ...
        `- Outcome: ${result.endReason}${result.abandonReason ? ` — ${result.abandonReason}` : ""}`,
        ...result.scores
            .filter((score) => score.dimension.startsWith("conversation.") || score.dimension === "overall")
            .map((score) => `- ${score.dimension}: ${score.value}/100`),
    ].join("\n");
    return {
        markdown: truncate(markdown),
 ...
 */
export async function runApplicationMap(input) {
    const isMock = input.url.startsWith("mock:");
    const browser = input.browser ?? (isMock ? "mock" : "playwright");
    const pool = input.personas.length > 0
        ? input.personas
...
export function listProfessionsTool() {
...
export function listCulturesTool() {
    ...
    const markdown = [
        ...
        "",
        ...cultures.map((c) => `- **${c.locale}** (${c.name}) — ${c.readingDirection.toUpperCase()}, ` +
...
export async function runEveBenchTool(input) {
...
export async function getReport(input) {
    ...
    try {
        ...
        throw new ToolInputError(`No ${file} found in "${input.output_dir}". Run eve_run_session with ` +
