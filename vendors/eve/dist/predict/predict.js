...
function proportion(metric, successes, n, note) {
...
const isFirstTimer = (op) => /first-?time|new-?user|onboard/i.test(op.persona);
const isAccessibility = (op) => /accessib|elderly|low-?vision|screen-?reader/i.test(op.persona);
const isConfused = (op) => op.segment === "confused-wanderers" || op.emotions.confusion >= 0.5;
/**
 * Predict the UX the wider user base will experience from a population study.
 */
export function predictUX(study) {
    ...
    if (firstTimers.length) {
        ...
        predictions.push(proportion("Onboarding failure rate", failures, firstTimers.length, "First-time users predicted to fail to activate"));
    }
    ...
    if (a11yOps.length) {
        ...
        predictions.push(proportion("Accessibility-barrier rate", failures, a11yOps.length, "Accessibility-sensitive users predicted to hit a barrier"));
    }
    ...
    const struggleForecasts = study.navigationHeatmap
        .map((e) => {
        ...
        const reasons = [];
        if (e.dropoffs)
            reasons.push(`${e.dropoffs} drop-off(s)`);
        if (revisit >= 3)
            reasons.push(`${round(revisit, 1)}× revisits/user`);
        return {
