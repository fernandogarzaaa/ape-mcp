...
export function safeJoin(dir, ...segments) {
 ...
 */
export function assertUrlAllowed(url, allowlist) {
    ...
    const ok = allowlist.some((h) => host === h.toLowerCase() || host.endsWith(`.${h.toLowerCase()}`));
    if (!ok)
