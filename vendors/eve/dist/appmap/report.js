...
export function renderApplicationMapMermaid(map) {
    const ids = new Map();
...
export function renderApplicationMapMarkdown(map) {
    const lines = [
    ...
    for (const s of map.screens) {
        lines.push(`- **${shortName(s.id)}** — _${s.purpose}_ · ${s.affordances.length} affordances, ${s.inDegree} in / ${s.outDegree} out${map.entryPoints.includes(s.id) ? " · entry point" : ""}${map.deadEnds.includes(s.id) ? " · dead-end" : ""}`);
