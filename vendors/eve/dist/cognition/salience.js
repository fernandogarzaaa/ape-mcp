...
export function riskOf(element) {
    if (DESTRUCTIVE_RE.test(element.text))
        return 1;
    if (COMMITTING_RE.test(element.text))
        return 0.55;
    return 0;
}
/** Visual prominence: size, position, being a "real" control. */
export function prominenceOf(element, percept) {
    ...
    const centrality = 1 -
        Math.min(1, Math.abs(element.box.x + element.box.width / 2 - percept.viewport.width / 2) /
            (percept.viewport.width || 1));
    const roleBoost = element.role === "button"
        ? 0.3
        : element.role === "link"
            ? 0.15
...
export function goalRelevanceOf(element, goalKeywords) {
...
export function scoreAffordances(ctx, goalKeywords) {
    ...
    for (const element of percept.elements) {
        if (!element.interactive || element.disabled)
            continue;
        if (element.box.width < 2 || element.box.height < 2)
            continue;
...
export function readingLoad(percept) {
    let words = 0;
...
export function choiceLoad(percept) {
