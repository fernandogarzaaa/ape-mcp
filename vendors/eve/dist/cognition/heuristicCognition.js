 ...
 */
export class HeuristicCognition {
    strategy;
    name = "heuristic";
    constructor(strategy = "curious") {
    ...
    async decide(ctx) {
        ...
        if (!alreadyRead && memory.isNovelScreen(percept)) {
            ...
            const words = percept.elements.reduce((n, el) => n + el.text.split(/\s+/).filter(Boolean).length, 0);
        ...
        const goalHit = preScored.find((s) => s.goalRelevance >= 0.5 && s.novelty > 0 && !s.element.editable && s.risk < 0.6);
        if (goalHit) {
        ...
        if (affordanceDecision)
            return affordanceDecision;
     ...
     */
    chooseAffordance(ctx, goalKeywords, weights, effortBase, sig) {
        ...
        const considered = viable.slice(0, Math.max(1, Math.round(2 + (1 - persona.traits.attentionSpan) * 4)));
        const chosen = rng.weightedPick(considered, considered.map((s) => Math.exp(s.total * (1 + weights.goalWeight))));
        ...
        const choiceSet = heuristicChoiceSet(considered, considered.indexOf(chosen), `salience order, top-${considered.length} of ${viable.length} viable ` +
            `(attention-limited); weighted pick exp(total*(1+goalWeight))`, refused);
        ...
        const hesitant = chosen.risk > 0.4 && (persona.traits.riskTolerance < 0.4 || emotion.confidence < 0.4);
    ...
    handleToolSurface(ctx, sig) {
        ...
        const triedKey = (id) => `mcp-tried:${id}`;
        const untried = tools.filter((t) => !thoughts().includes(triedKey(t.id)));
        if (untried.length === 0)
            return null; // explored everything → cascade tail
        const goalKeywords = [...new Set([...goals.current.keywords, ...goals.root.keywords])];
        ...
        const schema = chosen.state.metadata?.inputSchema;
     ...
     */
    handleDocumentSurface(ctx, sig) {
        const kernel = ctx.kernel;
        if (kernel?.modality !== "document")
            return null;
        const { persona, emotion, memory, goals, rng } = ctx;
        ...
        const words = kernel.blocks.reduce((total, block) => total + block.text.split(/\s+/).filter(Boolean).length, 0);
        const gaps = kernel.signals.filter((s) => s.type === "comprehension-gap");
        ...
        const end = kernel.signals.find((s) => s.type === "end-of-content");
        if (end) {
            const timesHere = memory.knownScreens().find((screen) => screen.signature === sig)?.visits ?? 1;
            if (timesHere <= 1) {
                return {
        ...
        const studyTarget = kernel.affordances.find((a) => {
            if (!STUDYABLE.has(a.kind))
                return false;
            if (memory.currentThoughts().some((t) => t.content === `studied:${a.id}`))
                return false;
        ...
        const reference = kernel.affordances.find((a) => {
            if (a.kind !== "doc.reference")
                return false;
            if (memory.currentThoughts().some((t) => t.content === `followed:${a.id}`))
                return false;
            const tokens = tokenize(a.description);
            return goalKeywords.some((keyword) => tokens.includes(keyword));
        });
    ...
    handleConversationSurface(ctx) {
        ...
        const misunderstood = kernel.signals.find((s) => s.type === "not-understood");
        ...
        const persistence = clamp01((persona.traits.patience * 0.5 + persona.traits.resilience * 0.5) *
    ...
    handleDialog(ctx) {
        ...
        const affirmative = inDialog.find((el) => /\b(ok|okay|accept|agree|got it|continue|close|dismiss|allow|yes)\b/i.test(el.text));
    ...
    handleFormSubmit(ctx) {
        ...
        if (filledFields.length === 0)
            return null;
        const remainingFields = percept.elements.filter((el) => el.editable && !el.disabled && !node.triedAffordances.has(fieldKey(el)));
    ...
    handleFormField(ctx, goalKeywords) {
        ...
        const relevant = goalKeywords.some((kw) => emptyFields.some((f) => tokenize(f.text).includes(kw))) ||
            /\b(login|log in|sign|search|register|form|create|checkout|submit)\b/i.test(`${goalKeywords.join(" ")} ${percept.title}`);
        if (!formCentric && !relevant)
            return null;
...
function toolNameOf(affordance) {
...
function isFailureSignal(signal) {
 ...
 */
export function plausibleInput(field, personaName) {
    const label = field.text.toLowerCase();
    if (/e-?mail/.test(label))
        return `${personaName.replace(/[^a-z0-9]/g, ".")}@example.com`;
    if (/password/.test(label))
        return "CorrectHorse!42";
    if (/phone|tel/.test(label))
        return "555-0142";
    if (/search|find|query/.test(label))
        return "settings";
    if (/first\s*name/.test(label))
        return "Alex";
    if (/last\s*name|surname/.test(label))
        return "Rivera";
    if (/\bname\b|user/.test(label))
        return "Alex Rivera";
    if (/company|organization/.test(label))
        return "Acme Corp";
    if (/city/.test(label))
        return "Springfield";
    if (/zip|postal/.test(label))
        return "62704";
    if (/address/.test(label))
        return "742 Evergreen Terrace";
    if (/date|dob|birth/.test(label))
        return "1990-04-12";
    if (/url|website|link/.test(label))
        return "https://example.com";
    if (/title|subject/.test(label))
        return "A quick test";
    if (/comment|message|description|note/.test(label)) {
        return "Just trying this out to see how it works.";
    }
    if (/amount|price|number|quantity|qty/.test(label))
        return "3";
    return "test input";
}
/** Blocks a reader stops *on* — the things worth working out, not reading past. */
const STUDYABLE = new Set(["doc.table", "doc.figure", "doc.metric"]);
 ...
 * what it wanted.
 */
function rephrase(goal, attempt) {
