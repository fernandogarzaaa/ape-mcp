 ...
 */
export function synthesizeArguments(schema, personaName, rng) {
    const args = {};
    ...
    for (const [name, rawProp] of Object.entries(properties)) {
        ...
        if (!required.has(name) && !rng.chance(0.5))
            continue;
...
function valueForSchema(schema, name, personaName, rng) {
...
function plausibleNumber(name, schema) {
    const minimum = typeof schema.minimum === "number" ? schema.minimum : undefined;
    const maximum = typeof schema.maximum === "number" ? schema.maximum : undefined;
    let value = 3;
    if (/count|qty|quantity|amount|size|length|limit|num/i.test(name))
        value = 3;
    if (/year|age/i.test(name))
        value = 42;
    if (/percent|ratio|rate/i.test(name))
        value = 50;
 ...
 */
function plausibleString(name, schema, personaName) {
    ...
    if (/phone|tel/.test(label))
        return "555-0142";
    if (/search|find|query|\bq\b/.test(label))
        return "settings";
    ...
    if (/date|dob|birth/.test(label))
        return "1990-04-12";
    if (/url|website|link|uri/.test(label))
        return "https://example.com";
    if (/title|subject/.test(label))
        return "A quick test";
    if (/path|file|dir/.test(label))
    ...
    if (/amount|price|number|quantity|qty|count/.test(label))
        return "3";
    return "test input";
}
//# sourceMappingURL=toolArgs.js.map