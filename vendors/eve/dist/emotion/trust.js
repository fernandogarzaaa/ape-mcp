...
export class TrustModel {
     ...
     */
    update(outcome, perceivedError, gaveFeedback) {
        ...
        if (perceivedError)
            this.nudge("errorRecovery", -1);
        else if (outcome.screenChanged)
            this.nudge("errorRecovery", 0.2);
        // Latency beyond a couple seconds dents dependability-as-predictability.
        if (outcome.perceivedLatencyMs > 3000)
            this.nudge("predictability", -0.4);
    }
    ...
    overall() {
        ...
        return clamp01(c.predictability * 0.3 +
            c.consistency * 0.2 +
